...
translated (by machine 🤖) as:
```
Wählen Sie eine Offline-Kartendatei aus, die in den Offline-Kartenordner kopiert werden soll: %s
```
...
translated (by machine 🤖) as:
```
Offline-Kartendatei wird importiert...
```
...
translated (by machine 🤖) as:
```
Löschen Sie Dateien, die bekanntermaßen von der Kartenansicht verwendet werden und gelegentlich beschädigt werden, was dazu führt, dass die Kartenansicht einen leeren Hintergrund anzeigt.
```
...
translated (by machine 🤖) as:
```
Kartenansichtsdateien erfolgreich gelöscht. Die App wird nun beendet, um die Bereinigung abzuschließen.
```
...
translated (by machine 🤖) as:
```
Kartenansichtsdateien konnten nicht gelöscht werden
```
...
translated (by machine 🤖) as:
```
Das nächste große Update für Map Marker kommt bald! Diese neue Version wird wichtige Verbesserungen und neue Funktionen bringen.

Das App-Update bleibt kostenlos und optional. Klicken Sie auf „Mehr anzeigen“, um weitere Informationen zu erhalten.
```
...
translated (by machine 🤖) as:
```
Laufende Streckenaufzeichnungen
```
...
translated (by machine 🤖) as:
```
Verwenden Sie die native Google Maps-Ansicht
```
...
translated (by machine 🤖) as:
```
Dieser Anzeigemodus ist möglicherweise langsamer, führt aber weniger wahrscheinlich zu App-Abstürzen
```
...
translated (by machine 🤖) as:
```
(aktuelle Auswahl wird nicht angewendet, erfordert einen vollständigen Neustart der App)
```
