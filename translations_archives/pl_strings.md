...
translated (by machine 🤖) as:
```
Wybierz plik mapy offline do skopiowania do folderu map offline: %s
```
...
translated (by machine 🤖) as:
```
Wyczyść pliki, o których wiadomo, że są używane przez widok mapy i czasami ulegają uszkodzeniu, powodując, że widok mapy wyświetla puste tło.
```
...
translated (by machine 🤖) as:
```
Pliki widoku mapy zostały pomyślnie wyczyszczone. Aplikacja zostanie teraz zamknięta, aby zakończyć czyszczenie.
```
...
translated (by machine 🤖) as:
```
Nie udało się wyczyścić plików widoku mapy
```
...
translated (by machine 🤖) as:
```
Kolejna duża aktualizacja do Map Marker już wkrótce! Ta nowa wersja przyniesie ważne ulepszenia i nowe funkcje.

Aktualizacja aplikacji pozostanie bezpłatna i opcjonalna, kliknij „Zobacz więcej”, aby uzyskać więcej informacji.
```
...
translated (by machine 🤖) as:
```
Użyj natywnego widoku Google Maps
```
...
translated (by machine 🤖) as:
```
Ten tryb wyświetlania może działać wolniej, ale jest mniej prawdopodobne, że spowoduje awarię aplikacji
```
...
translated (by machine 🤖) as:
```
(aktualny wybór nie został zastosowany, wymaga ponownego uruchomienia aplikacji)
```
...
translated (by machine 🤖) as:
```
Wystąpił nieoczekiwany błąd uwierzytelniania Google. Wygląda na to, że lokalna pamięć podręczna urządzenia jest uszkodzona.
Mogło się tak zdarzyć, jeśli niedawno odwołałeś/zresetowałeś/usunąłeś połączenie między tą aplikacją a Twoim kontem Google.
Pamięć podręczna urządzenia powinna być odświeżana automatycznie co godzinę.
Odczekaj godzinę lub dwie i spróbuj ponownie.
```
