...
translated (by machine 🤖) as:
```
Wis bestanden waarvan bekend is dat ze worden gebruikt door de kaartweergave en die af en toe beschadigd raken, waardoor de kaartweergave een lege achtergrond weergeeft.
```
...
translated (by machine 🤖) as:
```
Kaartweergavebestanden zijn met succes gewist. De app wordt nu afgesloten om het opschonen te voltooien.
```
...
translated (by machine 🤖) as:
```
Kan bestanden met kaartweergave niet wissen
```
...
translated (by machine 🤖) as:
```
De volgende grote update van Map Marker komt eraan! Die nieuwe versie zal belangrijke verbeteringen en nieuwe functies met zich meebrengen.

De app-update blijft gratis en optioneel, klik op "Meer bekijken" voor meer informatie.
```
...
translated (by machine 🤖) as:
```
Gebruik native Google Maps-weergave
```
...
translated (by machine 🤖) as:
```
Deze weergavemodus is mogelijk trager, maar veroorzaakt minder vaak app-crashes
```
...
translated (by machine 🤖) as:
```
(huidige keuze niet toegepast, vereist een volledige herstart van de app)
```
