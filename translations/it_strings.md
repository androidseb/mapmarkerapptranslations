...
translated (by machine 🤖) as:
```
Dimensione visualizzazione testo
```
...
translated (by machine 🤖) as:
```
Per poter rimanere attiva in background durante la registrazione di una traccia, l'app dovrà disporre dell'autorizzazione di localizzazione "sempre".
```
...
translated (by machine 🤖) as:
```
metro quadrato (m²) / ettaro (ha) / chilometro quadrato (km²)
```
...
translated (by machine 🤖) as:
```
piede quadrato (ft²) / acro (ac) / miglio quadrato (mi²)
```
...
translated (by machine 🤖) as:
```
Sistema metrico: metri / chilometri
```
...
translated (by machine 🤖) as:
```
Sistema imperiale: Piedi / Miglia
```
...
translated (by machine 🤖) as:
```
La connessione al tuo account Google avviene in due fasi: autenticazione e autorizzazione.

Potresti visualizzare due popup consecutivi durante la procedura di connessione: in tal caso, assicurati di selezionare lo stesso account Google entrambe le volte.

%1$s Fase 1/2 - autenticazione

%2$s Fase 2/2 - autorizzazione
```
...
translated (by machine 🤖) as:
```
La registrazione del percorso è possibile solo se la posizione GPS è sufficientemente precisa. È possibile regolare la soglia di precisione dalle impostazioni dell'app.
```
...
translated (by machine 🤖) as:
```
Impossibile aprire la finestra popup, molto probabilmente bloccata dal browser. Controlla le impostazioni del browser per consentire i popup su questo sito. La maggior parte dei browser ha un pulsante "Consenti" che appare vicino alla barra degli indirizzi quando si bloccano i popup.
```
