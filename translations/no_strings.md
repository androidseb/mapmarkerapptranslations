...
translated (by machine 🤖) as:
```
Bruk den opprinnelige Google Maps-visningen
```
...
translated (by machine 🤖) as:
```
Denne visningsmodusen kan være tregere, men det er mindre sannsynlig at den forårsaker appkrasj
```
...
translated (by machine 🤖) as:
```
(nåværende valg ikke brukt, krever en fullstendig omstart av appen)
```
