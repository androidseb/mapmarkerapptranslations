# Map Marker translations for Norwegian (no)
...
translated (by machine 🤖) as:
```
På grunn av noen begrensninger i Googles autentiseringsflyt, er det ikke mulig å autentisere med et annet tillatelsesnivå (begrenset tilgang / full tilgang) med mindre du laster inn nettsiden på nytt. Utfør oppdateringshandlingen i nettleseren din og prøv igjen.
```
...
translated (by machine 🤖) as:
```
Fjern filer som er kjent for å brukes av kartvisningen og av og til bli ødelagt, noe som får kartvisningen til å vise en tom bakgrunn.
```
...
translated (by machine 🤖) as:
```
Kartvisningsfiler ble slettet. Appen avsluttes nå for å fullføre oppryddingen.
```
...
translated (by machine 🤖) as:
```
Kunne ikke slette kartvisningsfiler
```
...
translated (by machine 🤖) as:
```
Den neste store oppdateringen til Map Marker kommer snart! Den nye versjonen vil gi viktige forbedringer og nye funksjoner.

Appoppdateringen forblir gratis og valgfri, klikk på "Se mer" for å finne ut mer informasjon.
```
...
translated (by machine 🤖) as:
```
Ingen kartmappe funnet på skyen - hvis du leter etter gamle skykart laget med den gamle versjonen av Map Marker (V2), prøv alternativet "Åpne og migrér et V2-skykart" i stedet.
```
