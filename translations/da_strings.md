...
translated (by machine 🤖) as:
```
Brug den oprindelige Google Maps-visning
```
...
translated (by machine 🤖) as:
```
Denne visningstilstand er muligvis langsommere, men det er mindre sandsynligt, at den forårsager appnedbrud
```
...
translated (by machine 🤖) as:
```
(nuværende valg anvendes ikke, kræver en fuldstændig genstart af appen)
```
