...
translated (by machine 🤖) as:
```
Dette begrænser appen til kun at få adgang til filer, der er oprettet af appen selv på din Google Drive. Vælg denne mulighed, hvis du kun planlægger at synkronisere fra din Google-konto uden samarbejdspartnere. Hvis du er i tvivl, så vælg denne mulighed, du kan nemt ændre mening senere.
```
...
translated (by machine 🤖) as:
```
Dette begrænser appen til kun at få adgang til en app-dedikeret Dropbox-mappe. Hvis du er i tvivl, så vælg denne mulighed, du kan nemt ændre mening senere.
```
...
translated (by machine 🤖) as:
```
Dette giver appen fuld adgang til din Dropbox. Vælg denne mulighed, hvis du ønsker, at appen skal finde filer i hele din Dropbox.
```
...
translated (by machine 🤖) as:
```
Skriv mere for at afgrænse din søgning
```
...
translated (by machine 🤖) as:
```
Du kan søge efter steder med "%s". Denne søgning kræver en internetforbindelse.
```
...
translated (by machine 🤖) as:
```
Synkroniseringen blev afbrudt, fordi følgende fil kunne findes: %s. For at løse dette kan du enten gendanne den manglende fil manuelt eller ændre appens indstillinger til ikke at downloade medier under synkronisering.
```
...
translated (by machine 🤖) as:
```
Politik for synkronisering af mediedownload
```
...
translated (by machine 🤖) as:
```
Download alle medier under synkronisering
```
...
translated (by machine 🤖) as:
```
Begynd at skrive app-udførelseshændelseshistorik til logfilen
```
...
translated (by machine 🤖) as:
```
Stop med at skrive app-udførelseshændelseshistorik til logfilen, og slet logfilen
```
...
translated (by machine 🤖) as:
```
Eksporter logfilen, så den kan deles til fejlretningsformål
```
...
translated (by machine 🤖) as:
```
Indstillinger for gengivelsesydelse
```
...
translated (by machine 🤖) as:
```
Maksimalt antal tegnede POI'er
```
...
translated (by machine 🤖) as:
```
Maksimalt antal tegnede flydende markørtitler
```
...
translated (by machine 🤖) as:
```
Se muligheder for at få adgang til kortdata fra de ældre versioner af Map Marker.
```
...
translated (by machine 🤖) as:
```
Migration af kortdata lykkedes: Du skulle nu kunne bruge dine gamle kortdata på denne nye version af Map Marker.
```
...
translated (by machine 🤖) as:
```
Migrering af kortdata mislykkedes: Adgang til dine gamle kortdata er ikke mulig på denne nye version af Map Marker.
```
...
translated (by machine 🤖) as:
```
Bemærk, at de gamle kortdata forbliver uændrede og forbliver på din enhed, indtil du afinstallerer appen. Hvis du afinstallerer appen, slettes alle Map Marker appdata fra din enhed. Hvis du vil bruge den gamle version af Map Marker, kan du følge de tilgængelige instruktioner ved at klikke på indstillingen nedenfor.
```
...
translated (by machine 🤖) as:
```
Vælg et gammelt kort, som datamigreringen skal køres på igen
```
...
translated (by machine 🤖) as:
```
Gamle kortdata re-migrering lykkedes!
```
...
translated (by machine 🤖) as:
```
Genmigrering af gamle kortdata mislykkedes:
%s
```
...
translated (by machine 🤖) as:
```
Eksporter den valgte markør som en fil
```
...
translated (by machine 🤖) as:
```
Vil du prøve at importere en markør fra følgende tekstdata?
```
...
translated (by machine 🤖) as:
```
Vælg en mulighed for at gemme den eksporterede fil.
```
...
translated (by machine 🤖) as:
```
Den eksporterede fil er stor og skal gemmes med 'Gem som'.
```
...
translated (by machine 🤖) as:
```
Vil du afslutte og gemme dette spor?
```
...
translated (by machine 🤖) as:
```
Hvis du vil give denne tilladelse, skal du gå til denne applikations tilladelsesindstillinger under dine systemindstillinger.
```
...
translated (by machine 🤖) as:
```
Hvis du lader denne tilladelse være deaktiveret, vil sporoptagelse kun fungere, mens appen er synlig i forgrunden.
```
...
translated (by machine 🤖) as:
```
Appen skal vise en meddelelse for at forblive aktiv i baggrunden, mens den optager et spor.
```
...
translated (by machine 🤖) as:
```
Sporoptagelsen (%s) blev afbrudt, før den kunne gemmes. Vil du gemme det nu?
```
...
translated (by machine 🤖) as:
```
Vil du slette de ikke-gemte sporoptagelser for altid?
```
...
translated (by machine 🤖) as:
```
Dette spor havde ingen point registreret og er simpelthen blevet kasseret.
```
...
translated (by machine 🤖) as:
```
Skift punkter med forrige/næste-knapperne, flyt rundt på kortet for at placere punkterne og klik på '-' for at fjerne et punkt. Klik til sidst på Gem for at gemme det redigerede nummer.
```
...
translated (by machine 🤖) as:
```
Aktiver præmien fra en kode genereret fra en anden enhed
```
...
translated (by machine 🤖) as:
```
Du har aktiveret gæstepremiumadgangen på denne enhed
```
...
translated (by machine 🤖) as:
```
Generer en premium-kode for at aktivere præmien på en af dine andre enheder
```
...
translated (by machine 🤖) as:
```
Premium-kode blev genereret: %s
Brug denne kode fra din anden enhed til at aktivere præmien der.
```
...
translated (by machine 🤖) as:
```
Opretter et nyt tomt kort og anvender sikre indstillinger for at forhindre yderligere nedbrud
```
...
translated (by machine 🤖) as:
```
Den sikre opsætning blev anvendt og træder i kraft ved næste lancering. Du kan nu afslutte appen og afslutte den.
```
...
translated (by machine 🤖) as:
```
Eksporter lokalt tilgængelige kortdata
```
...
translated (by machine 🤖) as:
```
Advarsel: gendannelseseksporten vil være delvis, mindst én cloud-mediefil mangler, vil ikke blive downloadet og vil blive erstattet med en tom fil under eksporten.
```
...
translated (by machine 🤖) as:
```
Det ser ud til, at din browser ikke er fuldt kompatibel - kun Google Chrome er testet som fuldt kompatibel
...
translated (by machine 🤖) as:
```
Vis påmindelse om sikkerhedskopiering af data
```
...
translated (by machine 🤖) as:
```
Vis alle redigeringsformpunkter (langsommere)
```
...
translated (by machine 🤖) as:
```
Dette giver dig mulighed for at synkronisere til en WebDAV-server ved at konfigurere adgangsoplysninger.
```
...
translated (by machine 🤖) as:
```
Appen skal have tilladelsen "altid" til placering for at kunne forblive aktiv i baggrunden, mens den optager et nummer.
```
...
translated (by machine 🤖) as:
```
Kvadratfod (ft²) / Acre (ac) / Kvadratmil (mi²)
```
...
translated (by machine 🤖) as:
```
Sporregistrering afventer tilstrækkelig præcis GPS-placering. Du kan justere nøjagtighedsgrænsen i appindstillingerne.
```
...
translated (by machine 🤖) as:
```
Åbning af pop op-vindue mislykkedes, sandsynligvis blokeret af din browser. Tjek dine browserindstillinger for at tillade pop op-vinduer på dette websted. De fleste browsere har en 'tillad'-knap, der vises omkring adresselinjen, når pop op-vinduer blokeres.
```
