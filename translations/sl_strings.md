...
translated (by machine 🤖) as:
```
To omejuje aplikacijo na dostop samo do datotek, ki jih je ustvarila sama aplikacija na vašem Google Drive. Izberite to možnost, če nameravate sinhronizirati samo iz svojega Google Računa brez sodelavcev. Če ste v dvomih, izberite to možnost, kasneje si boste zlahka premislili.
```
...
translated (by machine 🤖) as:
```
To aplikaciji omogoči popoln dostop do vašega Google Drive. Izberite to možnost, če nameravate sodelovati z drugimi Google Računi.
```
...
translated (by machine 🤖) as:
```
Omejitev količine mobilnih podatkov
```
...
translated (by machine 🤖) as:
```
To omejuje aplikacijo na dostop samo do mape Dropbox, namenjene aplikaciji. Če ste v dvomih, izberite to možnost, kasneje si boste zlahka premislili.
```
...
translated (by machine 🤖) as:
```
To aplikaciji omogoči popoln dostop do vašega Dropbox. Izberite to možnost, če želite, da aplikacija najde datoteke v celotnem Dropbox.
```
...
translated (by machine 🤖) as:
```
Datoteke zemljevidov brez povezave
```
...
translated (by machine 🤖) as:
```
Počisti datoteke zemljevidov brez povezave
```
...
translated (by machine 🤖) as:
```
Vnesite več, da izboljšate iskanje
```
...
translated (by machine 🤖) as:
```
Sinhronizacija je bila prekinjena, ker ste našli naslednjo datoteko: %s. Če želite odpraviti to težavo, lahko ročno obnovite manjkajočo datoteko ali spremenite nastavitve aplikacije, da med sinhronizacijo ne prenašate predstavnosti.
```
...
translated (by machine 🤖) as:
```
Politika prenosa medijev za sinhronizacijo
```
...
translated (by machine 🤖) as:
```
Prenesite vse medije med sinhronizacijo
```
...
translated (by machine 🤖) as:
```
Izberite atribute, ki jih je treba upoštevati pri odstranjevanju dvojnikov. Če so izbrani vsi atributi, bodo odstranjeni samo popolnoma enaki podvojeni vnosi. Če ni izbran noben atribut, bodo podvojeni vnosi z isto lokacijo odstranjeni, ne glede na njihove druge atribute.
```
...
translated (by machine 🤖) as:
```
Začnite pisati zgodovino dogodkov izvajanja aplikacije v dnevniško datoteko
```
...
translated (by machine 🤖) as:
```
Prenehajte pisati zgodovino dogodkov izvajanja aplikacije v datoteko dnevnika in izbrišite datoteko dnevnika
```
...
translated (by machine 🤖) as:
```
Dostopajte do starih zemljevidov
```
...
translated (by machine 🤖) as:
```
Oglejte si možnosti za dostop do podatkov zemljevida iz starejših različic Map Marker.
```
...
translated (by machine 🤖) as:
```
Selitev podatkov zemljevida je uspela: zdaj bi morali imeti možnost uporabljati svoje stare podatke zemljevida v tej novi različici Map Marker.
```
...
translated (by machine 🤖) as:
```
Selitev podatkov zemljevida ni uspela: dostop do vaših starih podatkov zemljevida ni mogoč v tej novi različici Map Marker.
```
...
translated (by machine 🤖) as:
```
Upoštevajte, da stari podatki zemljevida ostanejo nespremenjeni in bodo ostali v vaši napravi, dokler ne odstranite aplikacije. Če odstranite aplikacijo, boste iz naprave izbrisali vse podatke aplikacije Map Marker. Če želite uporabiti staro različico Map Marker, lahko sledite navodilom, ki so na voljo s klikom na spodnjo možnost.
```
...
translated (by machine 🤖) as:
```
S tem se odpre povezava do centra za pomoč, ki pojasnjuje, kako lahko namestite staro različico Map Marker, ne da bi pri tem izgubili podatke aplikacije.
```
...
translated (by machine 🤖) as:
```
Odprite in preselite zemljevid oblaka V2
```
...
translated (by machine 🤖) as:
```
Izvozite izbrani marker kot datoteko
```
...
translated (by machine 🤖) as:
```
Uporabite sistemske nastavitve
```
...
translated (by machine 🤖) as:
```
Menjajte točke z gumboma prej/naslednji, premikajte zemljevid, da postavite točke, in kliknite '-', da odstranite točko. Na koncu kliknite Shrani, da shranite urejeno skladbo.
```
...
translated (by machine 🤖) as:
```
Najmanjša hitrost premikanja: %s
```
...
translated (by machine 🤖) as:
```
Aktivirajte premium s kodo, ustvarjeno iz druge naprave
```
...
translated (by machine 🤖) as:
```
V tej napravi ste aktivirali premijski dostop za goste
```
...
translated (by machine 🤖) as:
```
Ustvarite premium kodo, da aktivirate premium na eni od vaših drugih naprav
```
...
translated (by machine 🤖) as:
```
Premium koda je bila uspešno ustvarjena: %s
Uporabite to kodo iz svoje druge naprave, da tam aktivirate premium.
```
...
translated (by machine 🤖) as:
```
Ustvari nov prazen zemljevid in uporabi varne nastavitve za preprečevanje nadaljnjih zrušitev
```
...
translated (by machine 🤖) as:
```
Varna nastavitev je bila uporabljena in bo veljala ob naslednjem zagonu, zdaj lahko zapustite aplikacijo in jo prekinete.
```
...
translated (by machine 🤖) as:
```
Izvozite lokalno razpoložljive zemljevidne podatke
```
...
translated (by machine 🤖) as:
```
Opozorilo: obnovitveni izvoz bo delen, manjka vsaj ena medijska datoteka v oblaku, ne bo prenesena in bo med izvozom nadomeščena s prazno datoteko.
```
...
translated (by machine 🤖) as:
```
To vam omogoča sinhronizacijo s strežnikom WebDAV tako, da konfigurirate poverilnice za dostop.
```
...
translated (by machine 🤖) as:
```
Vnesite konfiguracijske vrednosti
```
...
translated (by machine 🤖) as:
```
Aplikacija bo potrebovala dovoljenje za lokacijo »vedno«, da bo lahko med snemanjem sledi ostala aktivna v ozadju.
```
...
translated (by machine 🤖) as:
```
Kvadratni meter (m²) / Hektar (ha) / Kvadratni kilometer (km²)
```
...
translated (by machine 🤖) as:
```
Kvadratni čevelj (ft²) / Aker (ac) / Kvadratna milja (mi²)
```
...
translated (by machine 🤖) as:
```
Povezovanje z vašim računom Google poteka v dveh korakih: preverjanje pristnosti in avtorizacija. Med potekom povezave se lahko prikažeta dve pojavni okni zaporedoma – v tem primeru se prepričajte, da obakrat izberete isti račun Google.

%1$s Korak 1/2 – preverjanje pristnosti

%2$s Korak 2/2 – avtorizacija
```
...
translated (by machine 🤖) as:
```
Beleženje sledi se izvaja ob upoštevanju dovolj natančne lokacije GPS. Prag natančnosti lahko prilagodite v nastavitvah aplikacije.
```
...
translated (by machine 🤖) as:
```
Odpiranje pojavnega okna ni uspelo, najverjetneje ga je blokiral vaš brskalnik. Preverite nastavitve brskalnika, da dovolite pojavna okna na tem spletnem mestu. Večina brskalnikov ima gumb »dovoli«, ki se prikaže okoli naslovne vrstice, ko blokirajo pojavna okna.
```
