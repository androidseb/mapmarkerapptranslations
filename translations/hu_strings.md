...
translated (by machine 🤖) as:
```
Az offline térképfájl importálása sikeres volt
```
...
translated (by machine 🤖) as:
```
Az offline térképfájl importálása nem sikerült:
%s
```
...
translated (by machine 🤖) as:
```
Milyen formátumot szeretne használni az importáláshoz?
```
...
translated (by machine 🤖) as:
```
Írjon be többet a keresés finomításához
```
...
translated (by machine 🤖) as:
```
A szinkronizálás megszakadt, mert a következő fájl található: %s. A probléma megoldásához manuálisan visszaállíthatja a hiányzó fájlt, vagy módosíthatja az alkalmazás beállításait úgy, hogy a szinkronizálás során ne töltsön le médiát.
```
...
translated (by machine 🤖) as:
```
Szinkronizálási médialetöltési szabályzat
```
...
translated (by machine 🤖) as:
```
Az összes média letöltése szinkronizálás közben
```
...
translated (by machine 🤖) as:
```
Szükség esetén töltse le a médiát a szinkronizálás után
```
...
translated (by machine 🤖) as:
```
Automatikus szinkronizálás a felhővel, ha változásokat észlel
```
...
translated (by machine 🤖) as:
```
Válassza ki azokat az attribútumokat, amelyeket figyelembe kell venni az ismétlődések eltávolításakor. Ha az összes attribútumot kiválasztja, csak a szigorúan azonos ismétlődő bejegyzések törlődnek. Ha nincs kiválasztva attribútum, akkor az azonos helyen lévő ismétlődő bejegyzések a többi attribútumtól függetlenül törlődnek.
```
...
translated (by machine 🤖) as:
```
%d ismétlődést találtunk. Biztosan törli ezeket az ismétlődéseket?
```
...
translated (by machine 🤖) as:
```
Futtassa újra az adatmigrációt
```
...
translated (by machine 🤖) as:
```
Válasszon ki egy régi térképet, amelyen újra le szeretné futtatni az adatmigrációt
```
...
translated (by machine 🤖) as:
```
A régi térképadatok újraköltöztetése sikerült!
```
...
translated (by machine 🤖) as:
```
Válassza ki az áttelepítendő V2 térképet
```
...
translated (by machine 🤖) as:
```
A régebbi V2 felhőtérkép migrációja sikerült!
```
...
translated (by machine 🤖) as:
```
Megpróbál importálni egy markert a következő szöveges adatokból?
```
...
translated (by machine 🤖) as:
```
Igen, vigyen az alkalmazás oldalára
```
...
translated (by machine 🤖) as:
```
Nem, köszönöm, ne kérdezz többet
```
...
translated (by machine 🤖) as:
```
Válasszon egy lehetőséget az exportált fájl mentéséhez.
```
...
translated (by machine 🤖) as:
```
Az exportált fájl nagy, és a „Mentés másként” paranccsal kell menteni.
```
...
translated (by machine 🤖) as:
```
Minimális helymeghatározási pontosság
```
...
translated (by machine 🤖) as:
```
Minimális távolság intervallum
```
...
translated (by machine 🤖) as:
```
Folyamatban lévő pálya rögzítése
```
...
translated (by machine 🤖) as:
```
Be szeretné fejezni és elmenteni ezt a számot?
```
...
translated (by machine 🤖) as:
```
Meg akarja adni ezt az engedélyt?
```
...
translated (by machine 🤖) as:
```
Ha meg szeretné adni ezt az engedélyt, nyissa meg az alkalmazás engedélybeállításait a rendszerbeállítások alatt.
```
...
translated (by machine 🤖) as:
```
Ha letiltja ezt az engedélyt, a műsorszám rögzítése csak akkor működik, ha az alkalmazás látható az előtérben.
```
...
translated (by machine 🤖) as:
```
Az alkalmazásnak értesítést kell megjelenítenie, hogy aktív maradjon a háttérben a műsorszám rögzítése közben.
```
...
translated (by machine 🤖) as:
```
A műsorszám(ok) rögzítése (%s) megszakadt, mielőtt elmenthette volna. Szeretné most menteni?
```
...
translated (by machine 🤖) as:
```
Örökre törölni szeretné a nem mentett műsorszám(ok) felvételét?
```
...
translated (by machine 🤖) as:
```
Ennek a számnak nem volt rögzített pontja, és egyszerűen eldobták.
```
...
translated (by machine 🤖) as:
```
Váltson a pontok között az előző/következő gombokkal, mozgassa a térképet a pontok elhelyezéséhez, majd kattintson a '-' gombra egy pont eltávolításához. Végül kattintson a Mentés gombra a szerkesztett szám mentéséhez.
```
...
translated (by machine 🤖) as:
```
Ez lehetővé teszi a WebDAV-kiszolgálóval való szinkronizálást a hozzáférési hitelesítő adatok konfigurálásával.
```
...
translated (by machine 🤖) as:
```
Adja meg a konfigurációs értékeket
```
...
translated (by machine 🤖) as:
```
Natív Google Maps nézet használata
```
...
translated (by machine 🤖) as:
```
Ez a megjelenítési mód lassabb lehet, de kisebb valószínűséggel okoz alkalmazásösszeomlásokat.
```
...
translated (by machine 🤖) as:
```
(a jelenlegi választás nincs alkalmazva, teljes alkalmazás-újraindítást igényel)
```
...
translated (by machine 🤖) as:
```
Az alkalmazásnak „mindig” helymeghatározási engedéllyel kell rendelkeznie ahhoz, hogy aktív maradjon a háttérben az útvonal rögzítése közben.
```
