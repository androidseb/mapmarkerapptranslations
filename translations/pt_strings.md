...
translated (by machine 🤖) as:
```
Usar visualização nativa Google Maps
```
...
translated (by machine 🤖) as:
```
Este modo de exibição pode ser mais lento, mas é menos provável que cause travamentos no aplicativo
```
...
translated (by machine 🤖) as:
```
(opção atual não aplicada, requer reinicialização completa do aplicativo)
```
