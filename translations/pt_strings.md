...
translated (by machine 🤖) as:
```
Ative o prêmio a partir de um código gerado em outro dispositivo
```
...
translated (by machine 🤖) as:
```
Você ativou o acesso premium de convidado neste dispositivo
```
...
translated (by machine 🤖) as:
```
Gere um código premium para ativar o prêmio em um de seus outros dispositivos
```
...
translated (by machine 🤖) as:
```
Código premium gerado com sucesso: %s
Use este código do seu outro dispositivo para ativar o premium lá.
```
...
translated (by machine 🤖) as:
```
Recuperação: configuração segura
```
...
translated (by machine 🤖) as:
```
Cria um novo mapa vazio e aplica configurações seguras para evitar novas falhas
```
...
translated (by machine 🤖) as:
```
A configuração segura foi aplicada e entrará em vigor na próxima inicialização. Agora você pode sair do aplicativo e encerrá-lo.
```
...
translated (by machine 🤖) as:
```
Exporte dados de mapas disponíveis localmente
```
...
translated (by machine 🤖) as:
```
Atenção: a exportação de recuperação será parcial, pelo menos um arquivo de mídia em nuvem estará faltando, não será baixado e será substituído por um arquivo vazio durante a exportação.
```
...
translated (by machine 🤖) as:
```
Parece que seu navegador não é totalmente compatível - apenas o Google Chrome foi testado como totalmente compatível
...
translated (by machine 🤖) as:
```
Isso permite que você sincronize com um servidor WebDAV configurando credenciais de acesso.
```
...
translated (by machine 🤖) as:
```
Insira valores de configuração
```
...
translated (by machine 🤖) as:
```
O aplicativo precisará ter a permissão de localização "sempre" para permanecer ativo em segundo plano durante a gravação de uma trilha.
```
...
translated (by machine 🤖) as:
```
Metro quadrado (m²) / Hectare (ha) / Quilômetro quadrado (km²)
```
...
translated (by machine 🤖) as:
```
Pé quadrado (ft²) / Acre (ac) / Milha quadrada (mi²)
```
...
translated (by machine 🤖) as:
```
Sistema métrico: metros / quilômetros
```
...
translated (by machine 🤖) as:
```
A conexão com sua conta Google é feita em duas etapas: autenticação e autorização.
Você poderá ver duas janelas pop-up seguidas durante o processo de conexão. Nesse caso, certifique-se de selecionar a mesma conta Google nas duas vezes.

%1$s Etapa 1/2 - autenticação

%2$s Etapa 2/2 - autorização
```
