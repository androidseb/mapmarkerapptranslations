...
translated (by machine 🤖) as:
```
Bạn có muốn tìm kiếm từ một nguồn khác?
```
...
translated (by machine 🤖) as:
```
Tự hỏi làm thế nào để cấu hình các nguồn tìm kiếm? Bấm vào đây.
```
...
translated (by machine 🤖) as:
```
Thêm hoặc định cấu hình nguồn tìm kiếm
```
...
translated (by machine 🤖) as:
```
Thử nghiệm cấu hình nguồn tìm kiếm đã thành công! Việc tìm kiếm với nguồn tìm kiếm này sẽ hoạt động.
```
...
translated (by machine 🤖) as:
```
Cảnh báo: kiểm tra cấu hình nguồn tìm kiếm không thành công. Tìm kiếm với nguồn tìm kiếm này có thể không hoạt động.
```
...
translated (by machine 🤖) as:
```
Trải nghiệm tìm kiếm mặc định không tối ưu, nhưng nó hoạt động mà không cần cấu hình.
...
translated (by machine 🤖) as:
```
Thư mục này có các thư mục con. Bạn có muốn xóa cả các thư mục con không? Tất cả các điểm đánh dấu liên quan đến các thư mục đã xóa sẽ bị xóa vĩnh viễn!
```
...
translated (by machine 🤖) as:
```
Bạn có chắc chắn muốn xóa các thư mục này không? Tất cả các điểm đánh dấu liên quan đến các thư mục này sẽ bị xóa vĩnh viễn!
```
...
translated (by machine 🤖) as:
```
Ít nhất một trong các thư mục cần xóa có các thư mục con. Bạn có muốn xóa cả các thư mục con không? Tất cả các điểm đánh dấu liên quan đến các thư mục đã xóa sẽ bị xóa vĩnh viễn!
```
...
translated (by machine 🤖) as:
```
Bạn không thể di chuyển một thư mục đến một trong các thư mục con của nó.
```
...
translated (by machine 🤖) as:
```
Không có nguồn tìm kiếm nào được định cấu hình vào lúc này. Để có thể tìm kiếm, bạn cần định cấu hình nguồn tìm kiếm của bên thứ ba.
...
translated (by machine 🤖) as:
```
Nhập thêm để tinh chỉnh tìm kiếm của bạn
```
...
translated (by machine 🤖) as:
```
Xem các tùy chọn để truy cập dữ liệu bản đồ từ các phiên bản cũ hơn của Map Marker.
```
...
translated (by machine 🤖) as:
```
Di chuyển dữ liệu bản đồ đã thành công: bây giờ bạn có thể sử dụng dữ liệu bản đồ cũ của mình trên phiên bản Map Marker mới này.
```
...
translated (by machine 🤖) as:
```
Di chuyển dữ liệu bản đồ không thành công: không thể truy cập vào dữ liệu bản đồ cũ của bạn trên phiên bản Map Marker mới này.
```
...
translated (by machine 🤖) as:
```
Lưu ý rằng dữ liệu bản đồ cũ vẫn không thay đổi và sẽ vẫn còn trên thiết bị của bạn cho đến khi bạn gỡ cài đặt ứng dụng. Gỡ cài đặt ứng dụng sẽ xóa tất cả dữ liệu ứng dụng Map Marker khỏi thiết bị của bạn. Nếu bạn muốn sử dụng phiên bản cũ của Map Marker, bạn có thể làm theo hướng dẫn có sẵn bằng cách nhấp vào tùy chọn bên dưới.
```
...
translated (by machine 🤖) as:
```
Thao tác này sẽ mở ra một liên kết trung tâm trợ giúp giải thích cách bạn có thể cài đặt phiên bản cũ của Map Marker mà không làm mất dữ liệu ứng dụng của mình.
```
...
translated (by machine 🤖) as:
```
Đang di chuyển bản đồ đám mây V2 cũ…
```
...
translated (by machine 🤖) as:
```
Xuất điểm đánh dấu đã chọn dưới dạng tệp
```
...
translated (by machine 🤖) as:
```
Bạn có muốn thử và nhập một điểm đánh dấu từ dữ liệu văn bản sau không?
```
...
translated (by machine 🤖) as:
```
Vâng, đưa tôi đến trang ứng dụng
```
...
translated (by machine 🤖) as:
```
Chọn một tùy chọn để lưu tệp đã xuất.
```
...
translated (by machine 🤖) as:
```
Tệp đã xuất có dung lượng lớn và phải được lưu bằng 'Save as'.
```
...
translated (by machine 🤖) as:
```
Bạn có muốn hoàn thành và lưu bản nhạc này không?
```
...
translated (by machine 🤖) as:
```
Nếu bạn muốn cấp quyền này, hãy đi tới cài đặt quyền của ứng dụng này trong cài đặt hệ thống của bạn.
```
...
translated (by machine 🤖) as:
```
Nếu bạn tắt quyền này, tính năng ghi bản nhạc sẽ chỉ hoạt động khi ứng dụng hiển thị ở nền trước.
```
...
translated (by machine 🤖) as:
```
Ứng dụng sẽ cần hiển thị thông báo để duy trì hoạt động ở chế độ nền trong khi ghi một bản nhạc.
```
...
translated (by machine 🤖) as:
```
(Các) bản ghi (%s) đã bị gián đoạn trước khi có thể lưu. Bạn có muốn lưu nó bây giờ không?
```
...
translated (by machine 🤖) as:
```
Bạn có muốn xóa vĩnh viễn (các) bản ghi chưa được lưu không?
```
...
translated (by machine 🤖) as:
```
Bài hát này không có điểm nào được ghi lại và đơn giản là đã bị loại bỏ.
```
...
translated (by machine 🤖) as:
```
Kích hoạt phí bảo hiểm từ mã được tạo từ thiết bị khác
```
...
translated (by machine 🤖) as:
```
Không thể kích hoạt phí bảo hiểm:
%s
```
...
translated (by machine 🤖) as:
```
Bạn đã kích hoạt quyền truy cập cao cấp dành cho khách trên thiết bị này
```
...
translated (by machine 🤖) as:
```
Tạo mã cao cấp để kích hoạt phí bảo hiểm trên một trong các thiết bị khác của bạn
```
...
translated (by machine 🤖) as:
```
Mã cao cấp được tạo thành công: %s
Sử dụng mã này từ thiết bị khác của bạn để kích hoạt phí bảo hiểm ở đó.
```
...
translated (by machine 🤖) as:
```
Có vẻ như trình duyệt của bạn không hoàn toàn tương thích - chỉ có Google Chrome được kiểm tra là hoàn toàn tương thích
Xem chi tiết về khả năng tương thích bên dưới:
...
translated (by machine 🤖) as:
```
Hiển thị lời nhắc sao lưu dữ liệu
```
...
translated (by machine 🤖) as:
```
Hiển thị tất cả các điểm chỉnh sửa hình dạng (chậm hơn)
```
...
translated (by machine 🤖) as:
```
Tính năng này cho phép bạn đồng bộ hóa với máy chủ WebDAV bằng cách cấu hình thông tin đăng nhập.
```
...
translated (by machine 🤖) as:
```
Sử dụng chế độ xem Google Maps gốc
```
...
translated (by machine 🤖) as:
```
Chế độ hiển thị này có thể chậm hơn nhưng ít có khả năng gây ra sự cố ứng dụng
```
...
translated (by machine 🤖) as:
```
(lựa chọn hiện tại không được áp dụng, yêu cầu khởi động lại toàn bộ ứng dụng)
```
...
translated (by machine 🤖) as:
```
Ứng dụng sẽ cần có quyền 'luôn luôn' truy cập vị trí để có thể hoạt động ở chế độ nền trong khi ghi lại một bản nhạc.
```
