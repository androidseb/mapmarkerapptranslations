...
translated (by machine 🤖) as:
```
Algolia Places Search API (giới hạn hàng ngày)
```
...
translated (by machine 🤖) as:
```
Algolia Places Search API (tài khoản cá nhân)
```
...
translated (by machine 🤖) as:
```
Bạn can chỉ chọn một tệp để nhập at a lần
```
...
translated (by machine 🤖) as:
```
Bạn can chỉ nhập tệp có phần mở rộng sau: %s
```
...
translated (by machine 🤖) as:
```
Di chuyển các thư mục vào thư mục
```
...
translated (by machine 🤖) as:
```
Nhập khóa truy cập bí mật của bạn
```
...
translated (by machine 🤖) as:
```
Luôn hiển thị thư mục mặc định
```
...
translated (by machine 🤖) as:
```
Bạn không thể thực hiện thay đổi trên tệp bản đồ này vì tệp đang ở chế độ chỉ đọc.
```
