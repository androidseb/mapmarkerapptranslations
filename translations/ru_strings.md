...
translated (by machine 🤖) as:
```
Выберите файл автономной карты, который нужно скопировать в папку автономных карт: %s
```
...
translated (by machine 🤖) as:
```
Очистить файлы просмотра карты
```
...
translated (by machine 🤖) as:
```
Удалите файлы, которые, как известно, используются представлением карты и иногда повреждаются, в результате чего представление карты отображает пустой фон.
```
...
translated (by machine 🤖) as:
```
Файлы представления карты успешно удалены. Приложение закроется, чтобы завершить очистку.
```
...
translated (by machine 🤖) as:
```
Не удалось очистить файлы представления карты
```
...
translated (by machine 🤖) as:
```
Map Marker Версия 3 скоро появится!
```
...
translated (by machine 🤖) as:
```
Скоро будет следующее крупное обновление для Map Marker! Эта новая версия принесет важные улучшения и новые функции.

Обновление приложения останется бесплатным и необязательным. Нажмите «Подробнее», чтобы узнать больше.
```
