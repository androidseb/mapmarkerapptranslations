...
translated (by machine 🤖) as:
```
Использовать собственный Google Maps вид
```
...
translated (by machine 🤖) as:
```
Этот режим отображения может быть медленнее, но он реже приводит к сбоям в работе приложения.
```
...
translated (by machine 🤖) as:
```
(текущий выбор не применен, требуется полный перезапуск приложения)
```
