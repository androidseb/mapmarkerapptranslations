...
translated (by machine 🤖) as:
```
Dodajte datoteku izvanmrežne karte
```
...
translated (by machine 🤖) as:
```
Uvoz datoteke izvanmrežne karte je uspio
```
...
translated (by machine 🤖) as:
```
Izvanmrežni uvoz datoteke karte nije uspio:
%s
```
...
translated (by machine 🤖) as:
```
Mapa izvanmrežnih karata je prazna. Da biste koristili izvanmrežne karte, otvorite postavke i dodajte barem jednu datoteku izvanmrežne karte.
```
...
translated (by machine 🤖) as:
```
Koji format želite koristiti za uvoz?
```
...
translated (by machine 🤖) as:
```
Google Drive - ograničen pristup
```
...
translated (by machine 🤖) as:
```
Ovo ograničava aplikaciju na pristup samo datotekama koje je kreirala sama aplikacija na vašem Google Drive. Odaberite ovu opciju ako namjeravate sinkronizirati samo sa svog Google računa bez suradnika. Ako ste u nedoumici, odaberite ovu opciju, kasnije se lako možete predomisliti.
```
...
translated (by machine 🤖) as:
```
Sinkronizacija je prekinuta jer je premašila ograničenje dopuštenog volumena podataka. Za povećanje ili promjenu ograničenja idite na postavke.
```
...
translated (by machine 🤖) as:
```
To ograničava aplikaciju na pristup samo mapi Dropbox namijenjenoj aplikaciji. Ako ste u nedoumici, odaberite ovu opciju, kasnije se lako možete predomisliti.
```
...
translated (by machine 🤖) as:
```
Ovo aplikaciji daje puni pristup vašem Dropbox. Odaberite ovu opciju ako želite da aplikacija pronađe datoteke u cijelom vašem Dropbox.
```
...
translated (by machine 🤖) as:
```
Izbrišite datoteke izvanmrežne karte
```
...
translated (by machine 🤖) as:
```
Upišite više da biste precizirali pretraživanje
```
...
translated (by machine 🤖) as:
```
Možete tražiti mjesta s "%s". Za ovo pretraživanje potrebna je internetska veza.
```
...
translated (by machine 🤖) as:
```
Hej, hvala na tvojoj znatiželji!
...
translated (by machine 🤖) as:
```
Sinkronizacija je prekinuta jer ste mogli pronaći sljedeću datoteku: %s. Da biste to riješili, možete ili ručno vratiti datoteku koja nedostaje ili promijeniti postavke aplikacije da ne preuzimaju medije tijekom sinkronizacije.
```
...
translated (by machine 🤖) as:
```
Politika preuzimanja medija za sinkronizaciju
```
...
translated (by machine 🤖) as:
```
Preuzmite sve medije tijekom sinkronizacije
```
...
translated (by machine 🤖) as:
```
Preuzmite medije nakon sinkronizacije kada je potrebno
```
...
translated (by machine 🤖) as:
```
Automatska sinkronizacija s oblakom kada se otkriju promjene
```
...
translated (by machine 🤖) as:
```
Odaberite atribute koje treba uzeti u obzir prilikom uklanjanja duplikata. Ako su odabrani svi atributi, samo će se potpuno identični dvostruki unosi ukloniti. Ako nijedan atribut nije odabran, dvostruki unosi s istom lokacijom bit će uklonjeni, bez obzira na njihove druge atribute.
```
...
translated (by machine 🤖) as:
```
Pronađeno %d duplikata. Jeste li sigurni da želite izbrisati te duplikate?
```
...
translated (by machine 🤖) as:
```
Maksimalan broj nacrtanih naslova plutajućeg markera
```
...
translated (by machine 🤖) as:
```
Pristupite podacima stare karte
```
...
translated (by machine 🤖) as:
```
Pogledajte opcije za pristup kartografskim podacima iz starijih verzija Map Marker.
```
...
translated (by machine 🤖) as:
```
Migracija podataka karte je uspjela: sada biste trebali moći koristiti svoje stare podatke karte na ovoj novoj verziji Map Marker.
```
...
translated (by machine 🤖) as:
```
Migracija podataka karte nije uspjela: pristup vašim starim podacima karte nije moguć na ovoj novoj verziji Map Marker.
```
...
translated (by machine 🤖) as:
```
Ovo će otvoriti vezu centra za pomoć koja objašnjava kako možete instalirati staru verziju Map Marker bez gubitka podataka aplikacije.
```
...
translated (by machine 🤖) as:
```
Zadnja sigurnosna kopija napravljena je prije %s dana
```
...
translated (by machine 🤖) as:
```
Migracija naslijeđene karte oblaka V2…
```
...
translated (by machine 🤖) as:
```
Odaberite kartu V2 za migraciju
```
...
translated (by machine 🤖) as:
```
Migracija naslijeđene karte oblaka V2 uspjela!
```
...
translated (by machine 🤖) as:
```
Kako želite koristiti izvezenu datoteku?
```
...
translated (by machine 🤖) as:
```
Odaberite središte ili rub kruga pomoću gumba na alatnoj traci i pomičite kartu za podešavanje. Na kraju kliknite na Spremi da biste spremili oblik.
```
...
translated (by machine 🤖) as:
```
Želite li pokušati uvesti oznaku iz sljedećih tekstualnih podataka?
```
...
translated (by machine 🤖) as:
```
Da, odvedi me na stranicu aplikacije
```
...
translated (by machine 🤖) as:
```
Odaberite opciju za spremanje izvezene datoteke.
```
...
translated (by machine 🤖) as:
```
Izvezena datoteka je velika i mora se spremiti pomoću opcije "Spremi kao".
```
...
translated (by machine 🤖) as:
```
Minimalni interval udaljenosti
```
...
translated (by machine 🤖) as:
```
Pokretanje novog snimanja zapisa
```
...
translated (by machine 🤖) as:
```
Želite li završiti i spremiti ovu stazu?
```
...
translated (by machine 🤖) as:
```
Ako želite dati ovo dopuštenje, idite na postavke dopuštenja ove aplikacije u postavkama sustava.
```
...
translated (by machine 🤖) as:
```
Ako ovo dopuštenje ostavite onemogućenim, snimanje zapisa radit će samo dok je aplikacija vidljiva u prvom planu.
```
...
translated (by machine 🤖) as:
```
Aplikacija će morati prikazati obavijest kako bi ostala aktivna u pozadini tijekom snimanja staze.
```
...
translated (by machine 🤖) as:
```
Snimanje zapisa(a) (%s) je prekinuto prije nego što se moglo spremiti. Želite li ga sada spremiti?
```
...
translated (by machine 🤖) as:
```
Želite li zauvijek izbrisati zapis(e) koji nije spremljen?
```
...
translated (by machine 🤖) as:
```
Ova staza nije imala zabilježenih bodova i jednostavno je odbačena.
```
...
translated (by machine 🤖) as:
```
Mijenjajte točke s prethodnim/sljedećim gumbima, pomičite kartu kako biste postavili točke i kliknite '-' za uklanjanje točke. Na kraju kliknite spremi kako biste spremili uređenu stazu.
```
...
translated (by machine 🤖) as:
```
%1$s sata %2$s minute %3$s sekundi
```
...
translated (by machine 🤖) as:
```
Aktivirajte premium iz koda generiranog s drugog uređaja
```
...
translated (by machine 🤖) as:
```
Aktivirali ste premium pristup za goste na ovom uređaju
```
...
translated (by machine 🤖) as:
```
Generirajte premium kod za aktivaciju premium na jednom od vaših drugih uređaja
```
...
translated (by machine 🤖) as:
```
Premium kod uspješno generiran: %s
Upotrijebite ovaj kod sa svog drugog uređaja da tamo aktivirate premium.
```
...
translated (by machine 🤖) as:
```
Stvara novu praznu kartu i primjenjuje sigurne postavke za sprječavanje daljnjih padova
```
...
translated (by machine 🤖) as:
```
Sigurno postavljanje je primijenjeno i stupit će na snagu pri sljedećem pokretanju, sada možete izaći iz aplikacije i prekinuti je.
```
...
translated (by machine 🤖) as:
```
Izvezite lokalno dostupne podatke karte
```
...
translated (by machine 🤖) as:
```
Upozorenje: izvoz za oporavak bit će djelomičan, nedostaje barem jedna medijska datoteka u oblaku, neće se preuzeti i bit će zamijenjena praznom datotekom tijekom izvoza.
```
...
translated (by machine 🤖) as:
```
Čini se da vaš preglednik nije u potpunosti kompatibilan - samo je Google Chrome testiran kao potpuno kompatibilan
Pogledajte pojedinosti o kompatibilnosti u nastavku:
...
translated (by machine 🤖) as:
```
Prikaži podsjetnik za sigurnosno kopiranje podataka
```
...
translated (by machine 🤖) as:
```
Aplikacija će morati imati dopuštenje za lokaciju "uvijek" kako bi ostala aktivna u pozadini tijekom snimanja trase.
```
...
translated (by machine 🤖) as:
```
Kvadratni metar (m²) / Hektar (ha) / Kvadratni kilometar (km²)
```
...
translated (by machine 🤖) as:
```
Kvadratna stopa (ft²) / Jutar (ac) / Kvadratna milja (mi²)
```
