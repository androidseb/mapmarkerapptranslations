...
translated (by machine 🤖) as:
```
Бажаєте спробувати імпортувати маркер із наведених нижче текстових даних?
```
...
translated (by machine 🤖) as:
```
Так, перейти на сторінку програми
```
...
translated (by machine 🤖) as:
```
Виберіть опцію для збереження експортованого файлу.
```
...
translated (by machine 🤖) as:
```
Експортований файл великий і його потрібно зберегти за допомогою «Зберегти як».
```
...
translated (by machine 🤖) as:
```
Мінімальна точність визначення місця розташування
```
...
translated (by machine 🤖) as:
```
Ви бажаєте закінчити та зберегти цей трек?
```
...
translated (by machine 🤖) as:
```
Якщо ви хочете надати цей дозвіл, перейдіть до налаштувань дозволів цієї програми в налаштуваннях системи.
```
...
translated (by machine 🤖) as:
```
Якщо ви залишите цей дозвіл вимкненим, запис треку працюватиме, лише коли програма відображається на передньому плані.
```
...
translated (by machine 🤖) as:
```
Додаток має відобразити сповіщення, щоб залишатися активним у фоновому режимі під час запису треку.
```
...
translated (by machine 🤖) as:
```
Запис доріжки (%s) було перервано до того, як її вдалося зберегти. Ви хочете зберегти це зараз?
```
...
translated (by machine 🤖) as:
```
Бажаєте назавжди видалити незбережені записи композицій?
```
...
translated (by machine 🤖) as:
```
Для цієї доріжки не було записаних очок, тому її просто скасовано.
```
...
translated (by machine 🤖) as:
```
Перемикайте точки за допомогою кнопок «попередня/наступна», пересувайте карту, щоб розмістити точки, і натисніть «-», щоб видалити точку. Нарешті натисніть «Зберегти», щоб зберегти відредагований трек.
```
...
translated (by machine 🤖) as:
```
%1$s години %2$s хвилини %3$s секунд
```
...
translated (by machine 🤖) as:
```
Активуйте преміум з коду, згенерованого з іншого пристрою
```
...
translated (by machine 🤖) as:
```
Ви активували гостьовий преміум-доступ на цьому пристрої
```
...
translated (by machine 🤖) as:
```
Створіть преміум-код, щоб активувати преміум-код на одному з ваших інших пристроїв
```
...
translated (by machine 🤖) as:
```
Преміум-код успішно створено: %s
Використовуйте цей код з іншого пристрою, щоб активувати там преміум.
```
...
translated (by machine 🤖) as:
```
Відновлення: безпечне налаштування
```
...
translated (by machine 🤖) as:
```
Створює нову порожню карту та застосовує безпечні налаштування, щоб запобігти подальшим збоям
```
...
translated (by machine 🤖) as:
```
Безпечне налаштування було застосовано та почне діяти під час наступного запуску, тепер ви можете вийти з програми та завершити її роботу.
```
...
translated (by machine 🤖) as:
```
Експортуйте локально доступні картографічні дані
```
...
translated (by machine 🤖) as:
```
Попередження: експорт відновлення буде частковим, принаймні один хмарний медіа-файл відсутній, не буде завантажений і замінений порожнім файлом під час експорту.
```
...
translated (by machine 🤖) as:
```
Схоже, ваш веб-переглядач не повністю сумісний – лише Google Chrome перевірено як повністю сумісний
Перегляньте деталі сумісності нижче:
...
translated (by machine 🤖) as:
```
Показати нагадування про резервне копіювання даних
```
...
translated (by machine 🤖) as:
```
Показати всі точки форми для редагування (повільніше)
```
...
translated (by machine 🤖) as:
```
Це дає змогу синхронізуватися із сервером WebDAV, налаштувавши облікові дані доступу.
```
...
translated (by machine 🤖) as:
```
Квадратний метр (m²) / Гектар (ha) / Квадратний кілометр (km²)
```
...
translated (by machine 🤖) as:
```
Квадратний фут (ft²) / Акр (ac) / Квадратна миля (mi²)
```
...
translated (by machine 🤖) as:
```
Підключення до вашого облікового запису Google здійснюється у два кроки: автентифікація та авторизація.
Під час підключення ви можете побачити два спливаючі вікна поспіль. У такому разі переконайтеся, що ви обидва рази вибрали той самий обліковий запис Google.

%1$s Крок 1/2 – автентифікація

%2$s Крок 2/2 – авторизація
```
