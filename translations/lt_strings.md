...
translated (by machine 🤖) as:
```
Šiuo metu nėra sukonfigūruotas paieškos šaltinis. Kad galėtumėte ieškoti, turite sukonfigūruoti trečiosios šalies paieškos šaltinį.
...
translated (by machine 🤖) as:
```
Sinchronizavimas buvo nutrauktas, nes buvo galima rasti šį failą: %s. Norėdami tai išspręsti, galite rankiniu būdu atkurti trūkstamą failą arba pakeisti programos nustatymus, kad sinchronizuojant medija neatsisiunčiama.
```
...
translated (by machine 🤖) as:
```
Sinchronizavimo medijos atsisiuntimo politika
```
...
translated (by machine 🤖) as:
```
Atsisiųskite visą mediją sinchronizavimo metu
```
...
translated (by machine 🤖) as:
```
Pasiekite senus žemėlapio duomenis
```
...
translated (by machine 🤖) as:
```
Žr. parinktis, kaip pasiekti žemėlapio duomenis iš senesnių Map Marker versijų.
```
...
translated (by machine 🤖) as:
```
Žemėlapio duomenų perkėlimas pavyko: dabar turėtumėte turėti galimybę naudoti senus žemėlapio duomenis šioje naujoje Map Marker versijoje.
```
...
translated (by machine 🤖) as:
```
Nepavyko perkelti žemėlapio duomenų: prieiga prie senų žemėlapio duomenų negalima naudojant šią naują Map Marker versiją.
```
...
translated (by machine 🤖) as:
```
Dar kartą paleiskite duomenų perkėlimą
```
...
translated (by machine 🤖) as:
```
Pasirinkite seną žemėlapį, kuriame vėl norite atlikti duomenų perkėlimą
```
...
translated (by machine 🤖) as:
```
Senų žemėlapių duomenų perkėlimas pavyko!
```
...
translated (by machine 🤖) as:
```
Bendrinkite arba eksportuokite
```
...
translated (by machine 🤖) as:
```
Eksportuokite pasirinktą žymeklį kaip failą
```
...
translated (by machine 🤖) as:
```
Ar norite pabandyti importuoti žymeklį iš toliau nurodytų teksto duomenų?
```
...
translated (by machine 🤖) as:
```
Taip, nukreipkite mane į programos puslapį
```
...
translated (by machine 🤖) as:
```
Pasirinkite parinktį, kad išsaugotumėte eksportuotą failą.
```
...
translated (by machine 🤖) as:
```
Eksportuotas failas yra didelis ir turi būti išsaugotas naudojant „Išsaugoti kaip“.
```
...
translated (by machine 🤖) as:
```
Minimalus vietos nustatymo tikslumas
```
...
translated (by machine 🤖) as:
```
Pradedamas naujo takelio įrašymas
```
...
translated (by machine 🤖) as:
```
Ar norite užbaigti ir išsaugoti šį takelį?
```
...
translated (by machine 🤖) as:
```
Jei norite suteikti šį leidimą, eikite į šios programos leidimų nustatymus sistemos nustatymuose.
```
...
translated (by machine 🤖) as:
```
Jei paliksite šį leidimą išjungtą, maršruto įrašymas veiks tik tada, kai programa bus matoma priekiniame plane.
```
...
translated (by machine 🤖) as:
```
Programa turės rodyti pranešimą, kad įrašant takelį išliktų aktyvi fone.
```
...
translated (by machine 🤖) as:
```
Takelio (-ų) įrašymas (%s) buvo nutrauktas prieš jį įrašant. Ar norite jį išsaugoti dabar?
```
...
translated (by machine 🤖) as:
```
Ar norite visam laikui ištrinti neišsaugotą (-us) takelį (-ius)?
```
...
translated (by machine 🤖) as:
```
Šiame takelyje nebuvo įrašytų taškų ir jis buvo tiesiog išmestas.
```
...
translated (by machine 🤖) as:
```
Perjunkite taškus naudodami ankstesnį / kitą mygtuką, perkelkite žemėlapį, kad nustatytumėte taškus, ir spustelėkite „-“, kad pašalintumėte tašką. Galiausiai spustelėkite išsaugoti, kad išsaugotumėte redaguotą takelį.
```
...
translated (by machine 🤖) as:
```
%1$s valandos %2$s minutės %3$s sekundės
```
...
translated (by machine 🤖) as:
```
Įveskite apskritimo spindulį (%s)
```
...
translated (by machine 🤖) as:
```
Naudokite aukščiausios kokybės kodą
```
...
translated (by machine 🤖) as:
```
Suaktyvinkite priemoką iš kodo, sugeneruoto iš kito įrenginio
```
...
translated (by machine 🤖) as:
```
Įveskite aukščiausios kokybės kodą
```
...
translated (by machine 🤖) as:
```
Nepavyko suaktyvinti priemokos:
%s
```
...
translated (by machine 🤖) as:
```
Šiame įrenginyje suaktyvinote aukščiausios klasės svečio prieigą
```
...
translated (by machine 🤖) as:
```
Sukurkite aukščiausios kokybės kodą
```
...
translated (by machine 🤖) as:
```
Sugeneruokite aukščiausios kokybės kodą, kad suaktyvintumėte priemoką viename iš kitų įrenginių
```
...
translated (by machine 🤖) as:
```
Sėkmingai sugeneruotas aukščiausios kokybės kodas: %s
Naudokite šį kodą iš kito įrenginio, kad suaktyvintumėte priemoką.
```
...
translated (by machine 🤖) as:
```
Panašu, kad jūsų naršyklė nėra visiškai suderinama – tik „Google Chrome“ išbandyta kaip visiškai suderinama
...
translated (by machine 🤖) as:
```
Tai leidžia sinchronizuoti su WebDAV serveriu konfigūruojant prieigos kredencialus.
```
...
translated (by machine 🤖) as:
```
Įveskite konfigūracijos reikšmes
```
...
translated (by machine 🤖) as:
```
Naudoti natyvų Google Maps rodinį
```
...
translated (by machine 🤖) as:
```
Šis rodymo režimas gali būti lėtesnis, tačiau mažiau tikėtina, kad programos strigs.
```
...
translated (by machine 🤖) as:
```
(dabartinis pasirinkimas netaikomas, reikia visiškai paleisti programą iš naujo)
```
...
translated (by machine 🤖) as:
```
Kad programa galėtų likti aktyvi fone įrašant maršrutą, jai reikės „visada“ vietos nustatymo leidimo.
```
