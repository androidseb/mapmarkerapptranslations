...
translated (by machine 🤖) as:
```
Μια ιδιαίτερη ευχαριστία στους ανθρώπους που με βοήθησαν
```
...
translated (by machine 🤖) as:
```
Ευχαριστώ ιδιαίτερα τους ανθρώπους που εργάστηκαν σκληρά για να βοηθήσουν με τις μεταφράσεις εφαρμογής:
%s
...
translated (by machine 🤖) as:
```
Θα προτιμούσατε να κάνετε αναζήτηση από άλλη πηγή;
```
...
translated (by machine 🤖) as:
```
Αναρωτιέστε πώς να διαμορφώσετε τις πηγές αναζήτησης; Κάντε κλικ ΕΔΩ.
```
...
translated (by machine 🤖) as:
```
Προσθήκη ή διαμόρφωση προέλευσης αναζήτησης
```
...
translated (by machine 🤖) as:
```
Εισαγάγετε τιμή για το πεδίο "%s"
```
...
translated (by machine 🤖) as:
```
Η δοκιμή διαμόρφωσης πηγής αναζήτησης ήταν επιτυχής! Η αναζήτηση με αυτήν την πηγή αναζήτησης πρέπει τώρα να λειτουργήσει.
```
...
translated (by machine 🤖) as:
```
Προειδοποίηση: η δοκιμή διαμόρφωσης πηγής αναζήτησης απέτυχε. Η αναζήτηση με αυτήν την πηγή αναζήτησης ενδέχεται να μην λειτουργήσει.
```
...
translated (by machine 🤖) as:
```
Η προεπιλεγμένη εμπειρία αναζήτησης δεν είναι η βέλτιστη, αλλά λειτουργεί χωρίς διαμόρφωση.
...
translated (by machine 🤖) as:
```
Αυτός ο φάκελος έχει υποφακέλους. Θέλετε επίσης να διαγράψετε τους υποφακέλους; Όλοι οι δείκτες που σχετίζονται με τους διαγραμμένους φακέλους θα διαγραφούν οριστικά!
```
...
translated (by machine 🤖) as:
```
Είστε βέβαιοι ότι θέλετε να διαγράψετε αυτούς τους φακέλους; Όλοι οι δείκτες που σχετίζονται με αυτούς τους φακέλους θα διαγραφούν οριστικά!
```
...
translated (by machine 🤖) as:
```
Τουλάχιστον ένας από τους φακέλους για διαγραφή έχει υποφακέλους. Θέλετε επίσης να διαγράψετε τους υποφακέλους; Όλοι οι δείκτες που σχετίζονται με τους διαγραμμένους φακέλους θα διαγραφούν οριστικά!
```
...
translated (by machine 🤖) as:
```
Δεν μπορείτε να μετακινήσετε ένα φάκελο σε έναν από τους απογόνους του.
```
...
translated (by machine 🤖) as:
```
Προσθήκη αρχείου χάρτη εκτός σύνδεσης
```
...
translated (by machine 🤖) as:
```
Η εισαγωγή αρχείου χάρτη εκτός σύνδεσης ολοκληρώθηκε με επιτυχία
```
...
translated (by machine 🤖) as:
```
Η εισαγωγή αρχείου χάρτη εκτός σύνδεσης απέτυχε:
%s
```
...
translated (by machine 🤖) as:
```
Ο φάκελος χαρτών εκτός σύνδεσης είναι κενός. Για να χρησιμοποιήσετε χάρτες εκτός σύνδεσης, ανοίξτε τις ρυθμίσεις και προσθέστε τουλάχιστον ένα αρχείο χάρτη εκτός σύνδεσης.
```
...
translated (by machine 🤖) as:
```
Βασικό μέγεθος καρφίτσας χάρτη
```
...
translated (by machine 🤖) as:
```
Η ταυτοποίηση απέτυχε. Παρακαλώ προσπάθησε ξανά.
```
...
translated (by machine 🤖) as:
```
Πληκτρολογήστε περισσότερα για να κάνετε πιο συγκεκριμένη την αναζήτησή σας
```
...
translated (by machine 🤖) as:
```
Μπορείτε να αναζητήσετε μέρη με "%s". Αυτή η αναζήτηση απαιτεί σύνδεση στο Διαδίκτυο.
```
...
translated (by machine 🤖) as:
```
Ο συγχρονισμός ματαιώθηκε επειδή βρέθηκε το ακόλουθο αρχείο: %s. Για να επιλύσετε αυτό το πρόβλημα, μπορείτε είτε να επαναφέρετε το αρχείο που λείπει με μη αυτόματο τρόπο είτε να αλλάξετε τις ρυθμίσεις της εφαρμογής ώστε να μην γίνεται λήψη πολυμέσων κατά τον συγχρονισμό.
```
...
translated (by machine 🤖) as:
```
Λήψη όλων των μέσων κατά τον συγχρονισμό
```
...
translated (by machine 🤖) as:
```
Μέγιστος αριθμός σημείων ενδιαφέροντος που έχουν σχεδιαστεί
```
...
translated (by machine 🤖) as:
```
Μέγιστος αριθμός τίτλων κυμαινόμενου δείκτη
```
...
translated (by machine 🤖) as:
```
Πρόσβαση σε δεδομένα παλιού χάρτη
```
...
translated (by machine 🤖) as:
```
Δείτε επιλογές για πρόσβαση σε δεδομένα χάρτη από τις παλαιότερες εκδόσεις του Map Marker.
```
...
translated (by machine 🤖) as:
```
Μετεγκατάσταση δεδομένων χάρτη…
```
...
translated (by machine 🤖) as:
```
Η μετεγκατάσταση δεδομένων χάρτη πέτυχε: θα πρέπει πλέον να μπορείτε να χρησιμοποιείτε τα παλιά σας δεδομένα χάρτη σε αυτήν τη νέα έκδοση του Map Marker.
```
...
translated (by machine 🤖) as:
```
Η μετεγκατάσταση δεδομένων χάρτη απέτυχε: η πρόσβαση στα παλιά δεδομένα χάρτη σας δεν είναι δυνατή σε αυτήν τη νέα έκδοση του Map Marker.
```
...
translated (by machine 🤖) as:
```
Αυτό θα ανοίξει έναν σύνδεσμο κέντρου βοήθειας που θα εξηγεί πώς μπορείτε να εγκαταστήσετε την παλιά έκδοση του Map Marker χωρίς να χάσετε τα δεδομένα της εφαρμογής σας.
```
...
translated (by machine 🤖) as:
```
Το τελευταίο αντίγραφο ασφαλείας έγινε πριν από %s ημέρες
```
...
translated (by machine 🤖) as:
```
Η μετεγκατάσταση χαρτών νέφους παλαιού τύπου V2 πέτυχε!
```
...
translated (by machine 🤖) as:
```
Η μετεγκατάσταση του χάρτη cloud παλαιού τύπου V2 απέτυχε:
%s
```
...
translated (by machine 🤖) as:
```
Εξαγάγετε τον επιλεγμένο δείκτη ως αρχείο
```
...
translated (by machine 🤖) as:
```
Χρησιμοποιήστε τις ρυθμίσεις συστήματος
```
...
translated (by machine 🤖) as:
```
Θέλετε να δοκιμάσετε να εισαγάγετε έναν δείκτη από τα ακόλουθα δεδομένα κειμένου;
```
...
translated (by machine 🤖) as:
```
Ναι, μεταβείτε στη σελίδα της εφαρμογής
```
...
translated (by machine 🤖) as:
```
Όχι, ευχαριστώ, μην ξαναρωτήσεις
```
...
translated (by machine 🤖) as:
```
Ορίστε μια επιλογή για να αποθηκεύσετε το εξαγόμενο αρχείο.
```
...
translated (by machine 🤖) as:
```
Το εξαγόμενο αρχείο είναι μεγάλο και πρέπει να αποθηκευτεί με «Αποθήκευση ως».
```
...
translated (by machine 🤖) as:
```
Έναρξη νέας ηχογράφησης κομματιού
```
...
translated (by machine 🤖) as:
```
Θέλετε να ολοκληρώσετε και να αποθηκεύσετε αυτό το κομμάτι;
```
...
translated (by machine 🤖) as:
```
Θέλετε να παραχωρήσετε αυτήν την άδεια;
```
...
translated (by machine 🤖) as:
```
Εάν θέλετε να εκχωρήσετε αυτήν την άδεια, μεταβείτε στις ρυθμίσεις αδειών αυτής της εφαρμογής στις ρυθμίσεις του συστήματός σας.
```
...
translated (by machine 🤖) as:
```
Εάν αφήσετε αυτήν την άδεια απενεργοποιημένη, η εγγραφή κομματιού θα λειτουργεί μόνο όταν η εφαρμογή είναι ορατή στο προσκήνιο.
```
...
translated (by machine 🤖) as:
```
Η εφαρμογή θα χρειαστεί να εμφανίσει μια ειδοποίηση για να παραμείνει ενεργή στο παρασκήνιο κατά την εγγραφή ενός κομματιού.
```
...
translated (by machine 🤖) as:
```
Η εγγραφή κομματιού (%s) διακόπηκε προτού μπορέσει να αποθηκευτεί. Θέλετε να το αποθηκεύσετε τώρα;
```
...
translated (by machine 🤖) as:
```
Θέλετε να διαγράψετε οριστικά τα μη αποθηκευμένα κομμάτια;
```
...
translated (by machine 🤖) as:
```
Αυτό το κομμάτι δεν είχε καταγραφεί σημεία και απλώς απορρίφθηκε.
```
...
translated (by machine 🤖) as:
```
%1$s ώρες %2$s λεπτά %3$s δευτερόλεπτα
```
...
translated (by machine 🤖) as:
```
Εισαγάγετε την ακτίνα κύκλου (%s)
```
...
translated (by machine 🤖) as:
```
Ενεργοποιήστε το premium από έναν κωδικό που δημιουργήθηκε από άλλη συσκευή
```
...
translated (by machine 🤖) as:
```
Απέτυχε η ενεργοποίηση του premium:
%s
```
...
translated (by machine 🤖) as:
```
Έχετε ενεργοποιήσει την premium πρόσβαση επισκέπτη σε αυτήν τη συσκευή
```
...
translated (by machine 🤖) as:
```
Δημιουργήστε έναν κωδικό premium για να ενεργοποιήσετε το premium σε μία από τις άλλες συσκευές σας
```
...
translated (by machine 🤖) as:
```
Ο κωδικός Premium δημιουργήθηκε με επιτυχία: %s
Χρησιμοποιήστε αυτόν τον κωδικό από την άλλη συσκευή σας για να ενεργοποιήσετε το premium εκεί.
```
...
translated (by machine 🤖) as:
```
Φαίνεται ότι το πρόγραμμα περιήγησής σας δεν είναι πλήρως συμβατό - μόνο το Google Chrome έχει δοκιμαστεί ως πλήρως συμβατό
...
translated (by machine 🤖) as:
```
Εμφάνιση υπενθύμισης δημιουργίας αντιγράφων ασφαλείας δεδομένων
```
...
translated (by machine 🤖) as:
```
Η εφαρμογή θα πρέπει να έχει την άδεια τοποθεσίας «πάντα» για να παραμένει ενεργή στο παρασκήνιο κατά την εγγραφή ενός ίχνους.
```
...
translated (by machine 🤖) as:
```
Τετραγωνικό μέτρο (m²) / Εκτάριο (ha) / Τετραγωνικό χιλιόμετρο (km²)
```
...
translated (by machine 🤖) as:
```
Τετραγωνικό πόδι (ft²) / Άκρ (ac) / Τετραγωνικό μίλι (mi²)
```
...
translated (by machine 🤖) as:
```
Η καταγραφή της διαδρομής απαιτεί επαρκώς ακριβή εντοπισμό θέσης GPS. Μπορείτε να προσαρμόσετε το όριο ακρίβειας από τις ρυθμίσεις της εφαρμογής.
```
...
translated (by machine 🤖) as:
```
Το άνοιγμα του αναδυόμενου παραθύρου απέτυχε, πιθανότατα μπλοκαρίστηκε από το πρόγραμμα περιήγησής σας. Ελέγξτε τις ρυθμίσεις του προγράμματος περιήγησής σας για να επιτρέψετε τα αναδυόμενα παράθυρα σε αυτόν τον ιστότοπο. Τα περισσότερα προγράμματα περιήγησης έχουν ένα κουμπί "επιτρέπεται" που εμφανίζεται γύρω από τη γραμμή διευθύνσεων κατά τον αποκλεισμό αναδυόμενων παραθύρων.
```
