...
translated (by machine 🤖) as:
```
השם של השדה המותאם אישית אינו יכול להיות ריק
```
...
translated (by machine 🤖) as:
```
תוהה כיצד להגדיר מקורות חיפוש? לחץ כאן.
```
...
translated (by machine 🤖) as:
```
בדיקת תצורת מקור החיפוש הצליחה! החיפוש עם מקור חיפוש זה אמור לפעול כעת.
```
...
translated (by machine 🤖) as:
```
אזהרה: בדיקת תצורת מקור החיפוש נכשלה. החיפוש עם מקור חיפוש זה עלול שלא לפעול.
```
...
translated (by machine 🤖) as:
```
חוויית החיפוש המוגדרת כברירת מחדל אינה אופטימלית, אך היא פועלת ללא תצורה.
...
translated (by machine 🤖) as:
```
אין מקור מוגדר כרגע. על מנת שתוכל לחפש, עליך להגדיר מקור חיפוש של צד שלישי.
...
translated (by machine 🤖) as:
```
ייבוא קובץ מפות לא מקוון הצליח
```
...
translated (by machine 🤖) as:
```
ייבוא קובץ המפה הלא מקוון נכשל:
%s
```
...
translated (by machine 🤖) as:
```
הסנכרון בוטל מכיוון שניתן למצוא את הקובץ הבא: %s. כדי לפתור זאת, אתה יכול לשחזר את הקובץ החסר באופן ידני או לשנות את הגדרות האפליקציה כדי לא להוריד מדיה במהלך הסנכרון.
```
...
translated (by machine 🤖) as:
```
הורד את כל המדיה במהלך הסנכרון
```
...
translated (by machine 🤖) as:
```
הורד מדיה לאחר סנכרון בעת הצורך
```
...
translated (by machine 🤖) as:
```
סנכרן עם הענן באופן אוטומטי כאשר מתגלים שינויים
```
...
translated (by machine 🤖) as:
```
בחר את התכונות שיש לקחת בחשבון בעת ביטול כפילויות. אם כל התכונות נבחרות, יוסרו רק ערכים כפולים זהים לחלוטין. אם לא נבחרו תכונות, ערכים כפולים עם אותו מיקום יוסרו, ללא קשר לתכונות האחרות שלהם.
```
...
translated (by machine 🤖) as:
```
נמצאו %d כפילויות. האם אתה בטוח שברצונך למחוק את הכפילויות האלה?
```
...
translated (by machine 🤖) as:
```
המספר המרבי של נקודות עניין שצוירו
```
...
translated (by machine 🤖) as:
```
המספר המרבי של כותרות סמנים צפים מצוירים
```
...
translated (by machine 🤖) as:
```
ראה אפשרויות לגישה לנתוני מפה מהגרסאות הישנות יותר של Map Marker.
```
...
translated (by machine 🤖) as:
```
העברת נתוני המפה הצליחה: כעת אתה אמור להיות מסוגל להשתמש בנתוני המפה הישנים שלך בגרסה החדשה הזו של Map Marker.
```
...
translated (by machine 🤖) as:
```
העברת נתוני מפה נכשלה: גישה לנתוני המפה הישנים שלך אינה אפשרית בגרסה החדשה הזו של Map Marker.
```
...
translated (by machine 🤖) as:
```
שים לב שנתוני המפה הישנים נשארים ללא שינוי ויישארו במכשיר שלך עד שתסיר את ההתקנה של האפליקציה. הסרת ההתקנה של האפליקציה תמחק את כל Map Marker נתוני האפליקציה מהמכשיר שלך. אם אתה רוצה להשתמש בגרסה הישנה של Map Marker, אתה יכול לעקוב אחר ההוראות הזמינות על ידי לחיצה על האפשרות למטה.
```
...
translated (by machine 🤖) as:
```
זה יפתח קישור למרכז העזרה המסביר כיצד תוכל להתקין את הגרסה הישנה של Map Marker מבלי לאבד את נתוני האפליקציה שלך.
```
...
translated (by machine 🤖) as:
```
העבר מפת ענן מדור קודם מ-Map Marker V2 (גרסת אפליקציה מלפני 2023) ופתח אותה כמפת ענן חדשה
```
...
translated (by machine 🤖) as:
```
מעביר את מפת הענן V2 מדור קודם...
```
...
translated (by machine 🤖) as:
```
בחר את מרכז המעגל או את קצה המעגל בעזרת לחצני סרגל הכלים והזז את המפה כדי להתאים אותו. לבסוף, לחץ על שמור כדי לשמור את הצורה.
```
...
translated (by machine 🤖) as:
```
האם ברצונך לנסות ולייבא סמן מנתוני הטקסט הבאים?
```
...
translated (by machine 🤖) as:
```
בחר אפשרות לשמור את הקובץ המיוצא.
```
...
translated (by machine 🤖) as:
```
הקובץ המיוצא גדול ויש לשמור אותו באמצעות 'שמור בשם'.
```
...
translated (by machine 🤖) as:
```
האם ברצונך לסיים ולשמור את המסלול הזה?
```
...
translated (by machine 🤖) as:
```
החלף נקודות עם הכפתורים הקודם/הבא, הזז את המפה כדי למקם את הנקודות ולחץ על '-' כדי להסיר נקודה. לבסוף לחץ על שמור כדי לשמור את הרצועה הערוכה.
```
...
translated (by machine 🤖) as:
```
הפעל את הפרמיה מקוד שנוצר ממכשיר אחר
```
...
translated (by machine 🤖) as:
```
הפעלת את גישת הפרימיום לאורחים במכשיר זה
```
...
translated (by machine 🤖) as:
```
צור קוד פרימיום כדי להפעיל את הפרימיום באחד מהמכשירים האחרים שלך
```
...
translated (by machine 🤖) as:
```
קוד פרימיום נוצר בהצלחה: %s
השתמש בקוד זה מהמכשיר השני שלך כדי להפעיל את הפרימיום שם.
```
...
translated (by machine 🤖) as:
```
יוצר מפה ריקה חדשה ומחיל הגדרות בטוחות כדי למנוע קריסות נוספות
```
...
translated (by machine 🤖) as:
```
ההגדרה הבטוחה הוחלה ותיכנס לתוקף בהשקה הבאה, כעת תוכל לצאת מהאפליקציה ולסיים אותה.
```
...
translated (by machine 🤖) as:
```
אזהרה: ייצוא השחזור יהיה חלקי, חסר לפחות קובץ מדיה בענן אחד, לא יוריד ויוחלף בקובץ ריק במהלך הייצוא.
```
...
translated (by machine 🤖) as:
```
נראה שהדפדפן שלך אינו תואם לחלוטין - רק Google Chrome נבדק כתואם מלא
ראה את פרטי התאימות למטה:
...
translated (by machine 🤖) as:
```
הצג את כל נקודות הצורה העריכה (איט יותר)
```
...
translated (by machine 🤖) as:
```
זה מאפשר לך לסנכרן עם שרת WebDAV על ידי הגדרת אישורי גישה.
```
...
translated (by machine 🤖) as:
```
האפליקציה תצטרך לקבל הרשאת מיקום 'תמיד' על מנת להישאר פעילה ברקע בזמן הקלטת רצועה.
```
...
translated (by machine 🤖) as:
```
מטר מרובע (m²) / דונם (ha) / קילומטר מרובע (km²)
```
...
translated (by machine 🤖) as:
```
רגל מרובע (ft²) / אקר (ac) / מייל מרובע (mi²)
```
...
translated (by machine 🤖) as:
```
התחברות לחשבון Google שלך מתבצעת בשני שלבים: אימות ואישור.
ייתכן שתראה שני חלונות קופצים ברצף במהלך תהליך החיבור - במקרה זה, ודא שאתה בוחר את אותו חשבון Google בשני המקרים.
