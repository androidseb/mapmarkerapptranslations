# Map Marker translations for Hebrew (iw)
...
translated (by machine 🤖) as:
```
זה יחפש סמנים כפולים עם אותם נתונים בדיוק למחוק את כפילויות. רק עותק אחד של כל סמן ייחודי יישאר לאחר ניקוי זה. האם ברצונך להמשיך?
```
...
translated (by machine 🤖) as:
```
Algolia Places Search API (מגבלה יומית)
```
...
translated (by machine 🤖) as:
```
Algolia Places Search API (חשבון אישי)
```
...
translated (by machine 🤖) as:
```
אתה יכול לבחור רק קובץ אחד לייבוא הזמן at a
```
...
translated (by machine 🤖) as:
```
הצג תמיד את תיקיית ברירת המחדל
```
...
translated (by machine 🤖) as:
```
אינך יכול לבצע שינויים בקובץ מפה זה מכיוון שהוא נמצא במצב קריאה בלבד.
```
...
translated (by machine 🤖) as:
```
בחר קובץ מפה לא מקוון להעתקה לתיקיית המפות הלא מקוונות: %s
```
...
translated (by machine 🤖) as:
```
העדכון הגדול הבא ל-Map Marker מגיע בקרוב! הגרסה החדשה הזו תביא שיפורים חשובים ותכונות חדשות.

עדכון האפליקציה יישאר בחינם ואופציונלי, לחץ על "ראה עוד" למידע נוסף.
```
