...
translated (by human 👀) as:
```
Неуспех при преименуване на папката, името на дестинацията вече съществува
```
...
translated (by human 👀) as:
```
Услугите за местоположениеса са забранени. За да получите текущото си местоположение, трябва да ги разрешите.
```
...
translated (by human 👀) as:
```
Имате незапазени промени, искате ли да ги запишете?
```
...
translated (by human 👀) as:
```
Позиционирайте картата, за да поставите маркер, където искате и натиснете OK, за да го поставите
```
...
translated (by human 👀) as:
```
Показване на бутони за мащабиране
```
...
translated (by human 👀) as:
```
Отворете помощната страница и вижте "Често задавани въпроси" (FAQ) 
```
...
translated (by human 👀) as:
```
Premium достъп е на разположение! В момента имате достъп до първокласни функции.
```
...
translated (by human 👀) as:
```
Синхронизиране на тази карта с облака
```
...
translated (by human 👀) as:
```
Ъпгрейдвайте до премиум за да се възползвате напълно от Map Marker функциите!
```
...
translated (by human 👀) as:
```
За съжаление, трябва ъпгрейд до премиум за достъп до тази функция. Ъпгрейдвайте до премиум и се възползвайте напълно от Map Marker функциите!
```
...
translated (by human 👀) as:
```
Проверка, че приложението има достъп до интернет
```
...
translated (by human 👀) as:
```
Отворете подробностите на инсталираните в Google Play Services
```
...
translated (by human 👀) as:
```
Напишете email до поддръжката с подробностите на вашия проблем
```
...
translated (by human 👀) as:
```
Отворете папката на картата в облака за да видите свойствата и и да я споделите с други хора
```
...
translated (by human 👀) as:
```
Преглед на историята на изменения в папката на облака и избор на версия за да възстановяване. 
```
...
translated (by human 👀) as:
```
Създаване на нова карта локално или на облака
```
...
translated (by human 👀) as:
```
Не е намерена папка с карти на облака
```
...
translated (by human 👀) as:
```
Възникна грешка при синхронизирането: мрежова грешка. Моля, опитайте отново.
```
...
translated (by human 👀) as:
```
Възникна грешка при синхронизирането: конфликт, друго устройство извършва синхронизация в същото време. Моля, опитайте отново.
```
...
translated (by human 👀) as:
```
Възникна грешка при синхронизирането: данните в облака са актуализирани с по-нова версия на приложението. Трябва да обновите Map Marker на това устройство, за да синхронизирате.
```
...
```
 н
Навигационно действие по подразбиране 
```
...
translated (by human 👀) as:
```
Файлът вече съществува, искаш ли да бъде презаписан?
```
...
translated (by human 👀) as:
```
Кликнете тук, за да отворите уеб страницата, където се обяснява как да се получат тези карти.
```
...
translated (by human 👀) as:
```
Авторско право и правни аспекти
```
...
translated (by human 👀) as:
```
Това действие ще изтрие файла с карта от това устройство
```
...
translated (by human 👀) as:
```
Това действие ще изтрие файла на картата от устройството Ви завинаги. Всички маркери вътре в този файл ще бъдат загубени. Сигурни ли сте?
```
...
translated (by human 👀) as:
```
Това действие ще изтрие файла на картата от устройството Ви завинаги. Всички промени, които все още не са синхронизирани с облака, ще бъдат загубени. Папката в облака няма да бъде засегната от това действие и ще трябва да я изтриете ръчно. Сигурни ли сте?
```
...
translated (by human 👀) as:
```
Маркерът е записан в невидима папка
```
...
translated (by human 👀) as:
```
Маркерите на тази карта в момента се съхраняват само на това устройство. Препоръчително е да съхраните маркерите си на облака, за да се избегне загуба. Искате ли да ги съхраните в облака сега?
```
...
translated (by human 👀) as:
```
Искате ли да се синхронизира тази карта? С избор на "ръчно" ще трябва да синхронизирате ръчно всеки път. С "автоматично" (препоръчително) ще изберете папка-дестинация в облака и синхронизирането в облака ще се стартира автоматично всеки път, когато приложението влиза във фон.
```
...
translated (by human 👀) as:
```
Преместете картата, за да поставите точките, кликнете върху "+", за да добавите точка, кликнете върху "-", за да премахнете точка и точките на превключване с бутони "предишна/следваща". Накрая кликнете съхрани, за да запазите формата.
```
...
translated (by human 👀) as:
```
Бета тестер значка: Благодаря ви за това, че сте бета тестер! Тази значка ще изчезне само след деинсталирането на приложението или изтриването на данните на приложението. Тази значка също ви позволява да ползвате без реклами приложението.
```
...
translated (by human 👀) as:
```
Вие разбирате английски и искате да помогнете с превода? Натиснете тук
```
...
translated (by human 👀) as:
```
Стандартен текстов формат за таблица. Не може да съдържа медии. Не може да съдържа линии или полигони.
```
...
translated (by human 👀) as:
```
Премахване на полето, ако има такова: %s
```
...
%s
… А също и тези, които отказаха да бъдат споменати
...
Това ще отнеме само малко от времето Ви, но ще Ви позволи да се възползвате от много по-добри резултати от търсенето безплатно.
...
translated (by human 👀) as:
```
Импортирането на офлайн карта е успешно
```
...
translated (by human 👀) as:
```
Размер на щифта за основна карта
```
...
Ако не харесвате приложението, моля, изпратете ми имейл (от секцията „Относно“) и ме уведомете как мога да го подобря. Стигнах дотук благодарение на отзивите на много потребители, които ми казаха какво са пропуснали или не са харесали, и аз отговарям на всеки един имейл, така че не се срамувайте!

И накрая, благодаря ви, че прочетохте това :-) и за разбирането
...
translated (by human 👀) as:
```
Въведете навигационен режим в това приложение, за да получите указания към маркера
```
...
translated (by human 👀) as:
```
Ако ви харесва моето приложение можете да ме възнаградите, като отделите време да дадете добър рейтинг :-)
```
...
translated (by human 👀) as:
```
Записване на следа на местоположението
```
...
translated (by human 👀) as:
```
Стартиране на нов запис на следа
```
...
translated (by human 👀) as:
```
Приложението ще трябва да покаже известие, за да остане активно във фонов режим, докато записва следа.
```
...
translated (by human 👀) as:
```
Записването на следа(ите) (%s) беше прекъснато, преди да може да бъде запазено. Искате ли да го запазите сега?
```
...
translated (by human 👀) as:
```
Искате ли да изтриете завинаги записа на незаписаните следи?
```
...
translated (by human 👀) as:
```
Тази следа нямаше записани точки и просто беше изхвърлена.
```
...
translated (by human 👀) as:
```
Вижте информацията за поверителност
```
...
translated (by human 👀) as:
```
Превключете точките с бутоните за предишен/следващ, преместете картата наоколо, за да поставите точките и щракнете върху „-“, за да премахнете точка. Накрая щракнете върху запазване, за да запазите редактираната следа.
```
...
translated (by human 👀) as:
```
Приложението ще трябва да има разрешение за местоположение „винаги“, за да остане активно във фонов режим, докато записва следа.
```
...
translated (by human 👀) as:
```
Квадратен метър (m²) / Хектар (ha) / Квадратен километър (km²)
```
...
translated (by human 👀) as:
```
Квадратен фут (ft²) / Акър (ac) / Квадратна миля (mi²)
```
...
translated (by human 👀) as:
```
Метрична система: Метри / Километри
```
...
translated (by human 👀) as:
```
Свързването с вашия Google акаунт се осъществява чрез две стъпки: удостоверяване и оторизация.
...
translated (by machine 🤖) as:
```
Записването на трасето се извършва в очакване на достатъчно точно GPS местоположение. Можете да регулирате прага на точност от настройките на приложението.
```
...
translated (by machine 🤖) as:
```
Отварянето на изскачащ прозорец не бе успешно, най-вероятно е блокирано от браузъра ви. Проверете настройките на браузъра си, за да разрешите изскачащи прозорци на този сайт. Повечето браузъри имат бутон „разреши“, който се появява около адресната лента при блокиране на изскачащи прозорци.
```
