...
translated (by machine 🤖) as:
```
Kādu formātu vēlaties izmantot importēšanai?
```
...
translated (by machine 🤖) as:
```
Ierakstiet vairāk, lai precizētu meklēšanu
```
...
translated (by machine 🤖) as:
```
Sinhronizācija tika pārtraukta, jo tika atrasts šāds fails: %s. Lai to atrisinātu, varat manuāli atjaunot trūkstošo failu vai mainīt lietotnes iestatījumus, lai sinhronizācijas laikā netiktu lejupielādēta multivide.
```
...
translated (by machine 🤖) as:
```
Sinhronizācijas multivides lejupielādes politika
```
...
translated (by machine 🤖) as:
```
Sinhronizācijas laikā lejupielādējiet visu multividi
```
...
translated (by machine 🤖) as:
```
Ja nepieciešams, pēc sinhronizācijas lejupielādējiet multividi
```
...
translated (by machine 🤖) as:
```
Pēdējā dublēšana tika veikta pirms %s dienām
```
...
translated (by machine 🤖) as:
```
Palaidiet datu migrāciju vēlreiz
```
...
translated (by machine 🤖) as:
```
Atlasiet vecu karti, kurā vēlreiz palaist datu migrāciju
```
...
translated (by machine 🤖) as:
```
Veco karšu datu remigrācija izdevās!
```
...
translated (by machine 🤖) as:
```
Veco kartes datu atkārtota migrēšana neizdevās:
%s
```
...
translated (by machine 🤖) as:
```
Atveriet un migrējiet V2 mākoņa karti
```
...
translated (by machine 🤖) as:
```
Migrējiet mantoto mākoņa karti no Map Marker V2 (lietotnes versija pirms 2023. gada) un atveriet to kā jaunu mākoņa karti
```
...
translated (by machine 🤖) as:
```
Notiek mantotās V2 mākoņkartes migrēšana…
```
...
translated (by machine 🤖) as:
```
Mantotās V2 mākoņkartes migrācija ir izdevusies!
```
...
translated (by machine 🤖) as:
```
Eksportējiet atlasīto marķieri kā failu
```
...
translated (by machine 🤖) as:
```
Veiksmīgi saglabāts ierīces failos
```
...
translated (by machine 🤖) as:
```
Ar rīkjoslas pogām atlasiet apļa centru vai malu un pārvietojiet karti, lai pielāgotu. Visbeidzot, noklikšķiniet uz Saglabāt, lai saglabātu formu.
```
...
translated (by machine 🤖) as:
```
Vai vēlaties mēģināt importēt marķieri no tālāk norādītajiem teksta datiem?
```
...
translated (by machine 🤖) as:
```
Jā, novirziet mani uz lietotnes lapu
```
...
translated (by machine 🤖) as:
```
Izvēlieties iespēju saglabāt eksportēto failu.
```
...
translated (by machine 🤖) as:
```
Eksportētais fails ir liels, un tas ir jāsaglabā, izmantojot “Saglabāt kā”.
```
...
translated (by machine 🤖) as:
```
Minimālā atrašanās vietas precizitāte
```
...
translated (by machine 🤖) as:
```
Tiek sākta jauna ieraksta ierakstīšana
```
...
translated (by machine 🤖) as:
```
Vai vēlaties pabeigt un saglabāt šo ierakstu?
```
...
translated (by machine 🤖) as:
```
Skatiet informāciju par privātumu
```
...
translated (by machine 🤖) as:
```
Šī lietotne apkopo atrašanās vietas datus, lai iespējotu 'trases ierakstīšanas' funkciju, pat ja lietotne ir aizvērta vai netiek lietota.

Šī atrašanās vietas datu kolekcija tiek izmantota tikai, lai ierakstītu maršrutus jūsu marķieros, un tas notiek tikai tad, ja esat skaidri sācis maršrutu ierakstīšanu.

Plašāku informāciju par privātumu varat atrast privātuma politikā, kas pieejama šajā saitē:
%s
```
...
translated (by machine 🤖) as:
```
Pārslēdziet punktus, izmantojot iepriekšējās/nākamās pogas, pārvietojiet karti, lai novietotu punktus, un noklikšķiniet uz “-”, lai noņemtu punktu. Visbeidzot noklikšķiniet uz Saglabāt, lai saglabātu rediģēto ierakstu.
```
...
translated (by machine 🤖) as:
```
%1$s stundas %2$s minūtes %3$s sekundes
```
...
translated (by machine 🤖) as:
```
Aktivizējiet piemaksu no koda, kas ģenerēts no citas ierīces
```
...
translated (by machine 🤖) as:
```
Jūs esat aktivizējis viesa premium piekļuvi šajā ierīcē
```
...
translated (by machine 🤖) as:
```
Ģenerējiet premium kodu, lai aktivizētu piemaksu kādā no citām ierīcēm
```
...
translated (by machine 🤖) as:
```
Premium kods veiksmīgi ģenerēts: %s
Izmantojiet šo kodu no savas citas ierīces, lai tur aktivizētu premium.
```
...
translated (by machine 🤖) as:
```
Izveido jaunu tukšu karti un lieto drošus iestatījumus, lai novērstu turpmākas avārijas
```
...
translated (by machine 🤖) as:
```
Drošā iestatīšana tika lietota un stāsies spēkā nākamajā palaišanas reizē. Tagad varat iziet no lietotnes un pārtraukt to.
```
...
translated (by machine 🤖) as:
```
Eksportējiet lokāli pieejamos kartes datus
```
...
translated (by machine 🤖) as:
```
Brīdinājums: atkopšanas eksportēšana būs daļēja, trūkst vismaz viena mākoņa multivides faila, tas netiks lejupielādēts un eksportēšanas laikā tiks aizstāts ar tukšu failu.
```
...
translated (by machine 🤖) as:
```
Rādīt datu dublēšanas atgādinājumu
```
...
translated (by machine 🤖) as:
```
Parādīt visus rediģēšanas formas punktus (lēnāk)
```
...
translated (by machine 🤖) as:
```
Tas ļauj sinhronizēt ar WebDAV serveri, konfigurējot piekļuves akreditācijas datus.
```
...
translated (by machine 🤖) as:
```
Ievadiet konfigurācijas vērtības
```
...
translated (by machine 🤖) as:
```
Lai lietotne saglabātu aktivitāti fonā maršruta ierakstīšanas laikā, tai būs nepieciešama atrašanās vietas atļauja “vienmēr”.
```
...
translated (by machine 🤖) as:
```
Kvadrātmetrs (m²) / Hektārs (ha) / Kvadrātkilometrs (km²)
```
...
translated (by machine 🤖) as:
```
Kvadrātpēda (ft²) / Akrs (ac) / Kvadrātjūdze (mi²)
```
...
translated (by machine 🤖) as:
```
Metriskā mērvienība: metri/kilometri
```
...
translated (by machine 🤖) as:
```
Imperiālā mērvienība: pēdas/jūdzes
```
...
translated (by machine 🤖) as:
```
Maršruta ierakstīšana gaida pietiekami precīzu GPS atrašanās vietu. Precizitātes slieksni var pielāgot lietotnes iestatījumos.
```
...
translated (by machine 🤖) as:
```
Neizdevās atvērt uznirstošo logu, visticamāk, to ir bloķējusi jūsu pārlūkprogramma. Pārbaudiet pārlūkprogrammas iestatījumus, lai atļautu uznirstošos logus šajā vietnē. Lielākajai daļai pārlūkprogrammu, bloķējot uznirstošos logus, ap adreses joslu tiek parādīta poga “Atļaut”.
```
