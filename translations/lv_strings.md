...
translated (by machine 🤖) as:
```
Izmantot vietējo Google Maps skatu
```
...
translated (by machine 🤖) as:
```
Šis attēlošanas režīms varētu būt lēnāks, taču tas, visticamāk, neizraisīs lietotņu avārijas.
```
...
translated (by machine 🤖) as:
```
(pašreizējā izvēle netiek lietota, nepieciešama pilnīga lietotnes restartēšana)
```
