...
translated (by machine 🤖) as:
```
การทดสอบการกำหนดค่าแหล่งที่มาสำเร็จแล้ว! การค้นหาด้วยแหล่งค้นหานี้ควรใช้งานได้แล้ว
```
...
translated (by machine 🤖) as:
```
คำเตือน: การทดสอบการกำหนดค่าแหล่งค้นหาล้มเหลว การค้นหาด้วยแหล่งการค้นหานี้อาจไม่ทำงาน
```
...
translated (by machine 🤖) as:
```
ประสบการณ์การค้นหาเริ่มต้นไม่เหมาะสม แต่สามารถใช้งานได้โดยไม่มีการกำหนดค่า
...
translated (by machine 🤖) as:
```
โฟลเดอร์นี้มีโฟลเดอร์ย่อย คุณต้องการลบโฟลเดอร์ย่อยด้วยหรือไม่? เครื่องหมายทั้งหมดที่เกี่ยวข้องกับโฟลเดอร์ที่ถูกลบจะถูกลบอย่างถาวร!
```
...
translated (by machine 🤖) as:
```
แน่ใจไหมว่าต้องการลบโฟลเดอร์เหล่านี้ เครื่องหมายทั้งหมดที่เกี่ยวข้องกับโฟลเดอร์เหล่านี้จะถูกลบอย่างถาวร!
```
...
translated (by machine 🤖) as:
```
โฟลเดอร์ที่จะลบอย่างน้อยหนึ่งโฟลเดอร์มีโฟลเดอร์ย่อย คุณต้องการลบโฟลเดอร์ย่อยด้วยหรือไม่? เครื่องหมายทั้งหมดที่เกี่ยวข้องกับโฟลเดอร์ที่ถูกลบจะถูกลบอย่างถาวร!
```
...
translated (by machine 🤖) as:
```
คุณไม่สามารถย้ายโฟลเดอร์ไปยังโฟลเดอร์ที่สืบทอดมาได้
```
...
คลิกปุ่ม "เปลี่ยนแหล่งที่มา" จากนั้นคลิกตัวเลือก "บทช่วยสอน" เพื่อเรียนรู้เพิ่มเติม
```
...
translated (by machine 🤖) as:
```
การนำเข้าไฟล์แผนที่ออฟไลน์ล้มเหลว:
%s
```
...
translated (by machine 🤖) as:
```
โฟลเดอร์แผนที่ออฟไลน์ว่างเปล่า หากต้องการใช้แผนที่ออฟไลน์ ให้เปิดการตั้งค่าและเพิ่มไฟล์แผนที่ออฟไลน์อย่างน้อยหนึ่งไฟล์
```
...
translated (by machine 🤖) as:
```
สิ่งนี้จำกัดให้แอปเข้าถึงได้เฉพาะไฟล์ที่สร้างโดยแอปเองใน Google Drive ของคุณ เลือกตัวเลือกนี้หากคุณวางแผนที่จะซิงค์เฉพาะจากบัญชี Google ของคุณโดยไม่มีผู้ทำงานร่วมกัน หากไม่แน่ใจ ให้เลือกตัวเลือกนี้ คุณสามารถเปลี่ยนใจในภายหลังได้อย่างง่ายดาย
```
...
translated (by machine 🤖) as:
```
Google Drive - การเข้าถึงแบบเต็ม
```
...
translated (by machine 🤖) as:
```
สิ่งนี้ทำให้แอปเข้าถึง Google Drive ของคุณได้อย่างเต็มที่ เลือกตัวเลือกนี้หากคุณวางแผนที่จะทำงานร่วมกับบัญชี Google อื่นๆ
```
...
translated (by machine 🤖) as:
```
การดำเนินการนี้จะลบไฟล์แผนที่ออฟไลน์ทั้งหมดออกจากที่จัดเก็บข้อมูลภายในของแอป คุณต้องการดำเนินการต่อหรือไม่
```
...
translated (by machine 🤖) as:
```
พิมพ์เพิ่มเติมเพื่อปรับแต่งการค้นหาของคุณ
```
...
translated (by machine 🤖) as:
```
คุณสามารถค้นหาสถานที่ด้วย "%s" การค้นหานี้ต้องใช้การเชื่อมต่ออินเทอร์เน็ต
```
...
translated (by machine 🤖) as:
```
เฮ้ ขอบคุณสำหรับความอยากรู้ของคุณ!
...
translated (by machine 🤖) as:
```
กำลังโอนย้ายแผนที่ระบบคลาวด์ V2 เดิม…
```
...
translated (by machine 🤖) as:
```
ส่งออกเครื่องหมายที่เลือกเป็นไฟล์
```
...
translated (by machine 🤖) as:
```
บันทึกลงในไฟล์อุปกรณ์เรียบร้อยแล้ว
```
...
translated (by machine 🤖) as:
```
คุณต้องการลองและนำเข้าเครื่องหมายจากข้อมูลข้อความต่อไปนี้หรือไม่?
```
...
translated (by machine 🤖) as:
```
เลือกตัวเลือกเพื่อบันทึกไฟล์ที่ส่งออก
```
...
translated (by machine 🤖) as:
```
ไฟล์ที่ส่งออกมีขนาดใหญ่และต้องบันทึกด้วย "บันทึกเป็น"
```
...
translated (by machine 🤖) as:
```
คุณต้องการบันทึกแทร็กนี้ให้เสร็จหรือไม่
```
...
translated (by machine 🤖) as:
```
หากคุณต้องการให้สิทธิ์นี้ ให้ไปที่การตั้งค่าการอนุญาตของแอปพลิเคชันนี้ภายใต้การตั้งค่าระบบของคุณ
```
...
translated (by machine 🤖) as:
```
หากคุณปิดใช้งานการอนุญาตนี้ การบันทึกแทร็กจะทำงานเฉพาะในขณะที่แอปมองเห็นได้ในเบื้องหน้าเท่านั้น
```
...
translated (by machine 🤖) as:
```
แอปจะต้องแสดงการแจ้งเตือนเพื่อให้ยังคงทำงานอยู่เบื้องหลังขณะบันทึกแทร็ก
```
...
translated (by machine 🤖) as:
```
การบันทึกแทร็ก (%s) ถูกขัดจังหวะก่อนที่จะสามารถบันทึกได้ คุณต้องการบันทึกตอนนี้หรือไม่?
```
...
translated (by machine 🤖) as:
```
คุณต้องการลบการบันทึกแทร็กที่ยังไม่ได้บันทึกอย่างถาวรหรือไม่
```
...
translated (by machine 🤖) as:
```
แทร็กนี้ไม่มีการบันทึกคะแนนและถูกทิ้งไป
```
...
translated (by machine 🤖) as:
```
ดูข้อมูลเกี่ยวกับความเป็นส่วนตัว
```
...
translated (by machine 🤖) as:
```
แอปนี้รวบรวมข้อมูลตำแหน่งเพื่อเปิดใช้งานคุณลักษณะของ 'การบันทึกแทร็ก' แม้ว่าแอปจะปิดหรือไม่ได้ใช้งานก็ตาม

การรวบรวมข้อมูลตำแหน่งนี้ใช้เพื่อบันทึกเส้นทางลงในเครื่องหมายของคุณโดยเฉพาะ และจะเกิดขึ้นเมื่อคุณเริ่มการบันทึกเส้นทางอย่างชัดเจนเท่านั้น

คุณสามารถค้นหาข้อมูลทั่วไปเพิ่มเติมเกี่ยวกับความเป็นส่วนตัวได้ในนโยบายความเป็นส่วนตัวที่ลิงค์ต่อไปนี้:
%s
```
...
translated (by machine 🤖) as:
```
%1$s ชั่วโมง %2$s นาที %3$s วินาที
```
...
translated (by machine 🤖) as:
```
เปิดใช้งานพรีเมียมจากรหัสที่สร้างจากอุปกรณ์อื่น
```
...
translated (by machine 🤖) as:
```
ไม่สามารถเปิดใช้งานพรีเมี่ยมได้:
%s
```
...
translated (by machine 🤖) as:
```
คุณได้เปิดใช้งานการเข้าถึงแบบพรีเมียมของผู้มาเยือนบนอุปกรณ์นี้แล้ว
```
...
translated (by machine 🤖) as:
```
สร้างรหัสพรีเมียมเพื่อเปิดใช้งานพรีเมียมบนอุปกรณ์อื่นของคุณ
```
...
translated (by machine 🤖) as:
```
สร้างรหัสพรีเมี่ยมเรียบร้อยแล้ว: %s
ใช้รหัสนี้จากอุปกรณ์อื่นของคุณเพื่อเปิดใช้งานพรีเมียมที่นั่น
```
...
translated (by machine 🤖) as:
```
การกู้คืน: การตั้งค่าที่ปลอดภัย
```
...
translated (by machine 🤖) as:
```
สร้างแผนที่เปล่าใหม่และใช้การตั้งค่าที่ปลอดภัยเพื่อป้องกันการขัดข้องเพิ่มเติม
```
...
translated (by machine 🤖) as:
```
ใช้การตั้งค่าที่ปลอดภัยแล้ว และจะมีผลในการเปิดตัวครั้งถัดไป คุณสามารถออกจากแอปและยุติแอปได้แล้ว
```
...
translated (by machine 🤖) as:
```
ส่งออกข้อมูลแผนที่ที่มีอยู่ในท้องถิ่น
```
...
translated (by machine 🤖) as:
```
คำเตือน: การส่งออกการกู้คืนจะเป็นบางส่วน ไฟล์สื่อบนคลาวด์อย่างน้อยหนึ่งไฟล์หายไป จะไม่ถูกดาวน์โหลด และจะถูกแทนที่ด้วยไฟล์เปล่าระหว่างการส่งออก
```
...
translated (by machine 🤖) as:
```
แสดงการแจ้งเตือนการสำรองข้อมูล
```
...
translated (by machine 🤖) as:
```
แสดงจุดแก้ไขรูปร่างทั้งหมด (ช้าลง)
```
...
translated (by machine 🤖) as:
```
สิ่งนี้ช่วยให้คุณสามารถซิงค์กับเซิร์ฟเวอร์ WebDAV โดยการกำหนดค่าข้อมูลรับรองการเข้าถึง
```
...
translated (by machine 🤖) as:
```
แอปจะต้องมีสิทธิ์ระบุตำแหน่ง "เสมอ" เพื่อให้ยังคงทำงานในพื้นหลังขณะบันทึกแทร็ก
```
...
translated (by machine 🤖) as:
```
ตารางเมตร (m²) / เฮกตาร์ (ha) / ตารางกิโลเมตร (km²)
```
...
translated (by machine 🤖) as:
```
ตารางฟุต (ft²) / เอเคอร์ (ac) / ตารางไมล์(mi²)
```
...
translated (by machine 🤖) as:
```
หน่วยวัดแบบอิมพีเรียล: ฟุต / ไมล์
```
...
translated (by machine 🤖) as:
```
การบันทึกเส้นทางกำลังรอตำแหน่ง GPS ที่แม่นยำเพียงพอ คุณสามารถปรับเกณฑ์ความแม่นยำได้จากการตั้งค่าแอป
```
...
translated (by machine 🤖) as:
```
การเปิดหน้าต่างป๊อปอัพล้มเหลว อาจเป็นเพราะเบราว์เซอร์ของคุณบล็อกไว้ โปรดตรวจสอบการตั้งค่าเบราว์เซอร์ของคุณเพื่ออนุญาตให้แสดงป๊อปอัพบนเว็บไซต์นี้ เบราว์เซอร์ส่วนใหญ่จะมีปุ่ม "อนุญาต" ปรากฏอยู่รอบแถบที่อยู่เมื่อบล็อกป๊อปอัพ
```
