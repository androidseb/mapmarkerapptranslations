...
ขอขอบคุณสำหรับความอยากรู้ของคุณ! 
 ...
 หากคุณไม่ชอบแอปโปรด email ให้ฉัน (จากส่วน "เกี่ยวกับ") และแจ้งให้เราทราบว่าฉันสามารถปรับปรุงได้อย่างไร ฉันได้รับการตอบรับเป็นอย่างดีจากผู้ใช้จำนวนมากที่บอกฉันเกี่ยวกับสิ่งที่พวกเขาพลาดหรือไม่ชอบและฉันตอบกลับไปยัง email ทุกฉบับดังนั้นอย่ากลัวเลย! 
 
 และสุดท้ายขอขอบคุณสำหรับการอ่าน :-) 
...
translated (by machine 🤖) as:
```
Algolia Places Search API (จำกัด รายวัน)
```
...
translated (by machine 🤖) as:
```
Algolia Places Search API (บัญชีส่วนตัว)
```
...
translated (by machine 🤖) as:
```
คุณไม่สามารถทำการเปลี่ยนแปลงบนไฟล์แผนที่นี้ได้เนื่องจากอยู่ในโหมดอ่านอย่างเดียว
```
...
translated (by machine 🤖) as:
```
เลือกไฟล์แผนที่ออฟไลน์เพื่อคัดลอกไปยังโฟลเดอร์แผนที่ออฟไลน์: %s
```
...
translated (by machine 🤖) as:
```
เนื่องจากข้อจำกัดบางประการในกระบวนการตรวจสอบสิทธิ์ของ Google จึงเป็นไปไม่ได้ที่จะตรวจสอบสิทธิ์ด้วยระดับการอนุญาตอื่น (การเข้าถึงแบบจำกัด / การเข้าถึงแบบเต็ม) เว้นแต่คุณจะโหลดหน้าเว็บซ้ำ โปรดดำเนินการรีเฟรชในเว็บเบราว์เซอร์ของคุณและลองอีกครั้ง
```
...
translated (by machine 🤖) as:
```
ล้างไฟล์ที่ทราบว่าใช้โดยมุมมองแผนที่และเสียหายในบางครั้ง ทำให้มุมมองแผนที่แสดงพื้นหลังที่ว่างเปล่า
```
...
translated (by machine 🤖) as:
```
ล้างไฟล์มุมมองแผนที่เรียบร้อยแล้ว แอปจะออกเพื่อล้างข้อมูลให้เสร็จสิ้น
```
...
translated (by machine 🤖) as:
```
ปลายทางของไฟล์ที่ส่งออกคืออะไร
```
