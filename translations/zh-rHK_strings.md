# Translations for Chinese Traditional (zh-rHK)
...
translated (by machine 🤖) as:
```
選擇消除重複項時要考慮的屬性。如果選擇了所有屬性，則只會刪除嚴格相同的重複條目。如果未選擇任何屬性，則將刪除具有相同位置的重複條目，而不管它們的其他屬性如何。
```
...
translated (by machine 🤖) as:
```
地圖數據遷移成功：您現在應該可以在新版 Map Marker 上使用舊地圖數據了。
```
...
translated (by machine 🤖) as:
```
地圖數據遷移失敗：在這個新版本的 Map Marker 上無法訪問您的舊地圖數據。
```
...
translated (by machine 🤖) as:
```
請注意，舊地圖數據保持不變，並會保留在您的設備上，直到您卸載該應用程序。卸載應用程序將從您的設備中刪除所有 Map Marker 應用程序數據。如果您想使用舊版本的Map Marker，您可以點擊下面的選項按照可用的說明進行操作。
```
...
translated (by machine 🤖) as:
```
這將打開一個幫助中心鏈接，說明如何在不丟失應用數據的情況下安裝舊版本的 Map Marker。
```
...
translated (by machine 🤖) as:
```
從Map Marker V2（2023年之前的應用程序版本）遷移舊雲圖並將其作為新雲圖打開
```
...
translated (by machine 🤖) as:
```
使用工具欄按鈕選擇圓的中心或邊緣，然後四處移動地圖進行調整。最後點擊保存保存形狀。
```
...
translated (by machine 🤖) as:
```
如果您停用此權限，則僅當應用程式在前台可見時，軌跡記錄才會起作用。
```
...
translated (by machine 🤖) as:
```
即使應用程式關閉或未使用，該應用程式也會收集位置資料以啟用「軌跡記錄」功能。

此位置資料收集專門用於將軌跡記錄到標記中，並且僅在您明確啟動軌跡記錄時才會發生。

您可以在以下連結提供的隱私權政策中找到有關隱私的更多一般資訊：
%s
```
...
translated (by machine 🤖) as:
```
使用上一個/下一個按鈕切換點，移動地圖以放置點，然後按一下「-」刪除點。最後點選儲存即可儲存編輯好的曲目。
```
...
translated (by machine 🤖) as:
```
安全性設定已套用，並將在下次啟動時生效，您現在可以退出應用程式並終止它。
```
...
translated (by machine 🤖) as:
```
警告：恢復匯出將是部分的，至少遺失一個雲端媒體文件，將不會被下載，並在匯出過程中被替換為空文件。
```
...
translated (by machine 🤖) as:
```
您的瀏覽器似乎不完全相容 - 只有 Google Chrome 經測試完全相容
請參閱下面的相容性詳細資訊：
...
translated (by machine 🤖) as:
```
該應用程式需要具有「始終」位置權限才能在記錄軌跡時保持在背景處於活動狀態。
```
...
translated (by machine 🤖) as:
```
連接到您的Google帳戶需要兩個步驟：身份驗證和授權。

連接過程中，您可能會連續看到兩個彈出視窗 - 在這種情況下，請確保兩次都選擇同一個Google帳戶。
...
translated (by machine 🤖) as:
```
軌跡記錄需要 GPS 定位精度足夠高才能進行。您可以在應用程式設定中調整精度閾值。
```
...
translated (by machine 🤖) as:
```
打開彈出窗口失敗，很可能是被您的瀏覽器阻止了，請檢查您的瀏覽器設置以允許此網站彈出窗口，大多數瀏覽器在阻止彈出窗口時，地址欄附近都會出現一個“允許”按鈕。
```
