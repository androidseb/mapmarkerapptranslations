...
translated (by machine 🤖) as:
```
Adăugați un fișier de hartă offline
```
...
translated (by machine 🤖) as:
```
Importul fișierului de hartă offline a reușit
```
...
translated (by machine 🤖) as:
```
Importul fișierului hartă offline nu a reușit:
%s
```
...
translated (by machine 🤖) as:
```
Dimensiunea pinului hărții de bază
```
...
translated (by machine 🤖) as:
```
Tastați mai multe pentru a vă rafina căutarea
```
...
translated (by machine 🤖) as:
```
Hei, multumesc pentru curiozitate!
...
translated (by machine 🤖) as:
```
Accesați datele vechi ale hărții
```
...
translated (by machine 🤖) as:
```
Vedeți opțiunile pentru a accesa datele hărții din versiunile mai vechi ale Map Marker.
```
...
translated (by machine 🤖) as:
```
Migrarea datelor hărților a reușit: acum ar trebui să puteți utiliza datele vechi ale hărții pe această nouă versiune a Map Marker.
```
...
translated (by machine 🤖) as:
```
Migrarea datelor hărții a eșuat: accesul la datele dvs. vechi de hartă nu este posibil pe această nouă versiune a Map Marker.
```
...
translated (by machine 🤖) as:
```
Rețineți că datele vechi ale hărții rămân neschimbate și vor rămâne pe dispozitiv până când dezinstalați aplicația. Dezinstalarea aplicației va șterge toate cele Map Marker de date ale aplicației de pe dispozitiv. Dacă doriți să utilizați versiunea veche a Map Marker, puteți urma instrucțiunile disponibile făcând clic pe opțiunea de mai jos.
```
...
translated (by machine 🤖) as:
```
Aceasta va deschide un link către centrul de ajutor care explică cum puteți instala versiunea veche a Map Marker fără a pierde datele aplicației.
```
...
translated (by machine 🤖) as:
```
Deschideți și migrați o hartă cloud V2
```
...
translated (by machine 🤖) as:
```
Migrați o hartă cloud moștenită din Map Marker V2 (versiunea aplicației de înainte de 2023) și deschideți-o ca o nouă hartă cloud
```
...
translated (by machine 🤖) as:
```
Exportați marcatorul selectat ca fișier
```
...
translated (by machine 🤖) as:
```
Cum doriți să utilizați fișierul exportat?
```
...
translated (by machine 🤖) as:
```
Selectați centrul sau marginea cercului cu butoanele din bara de instrumente și mutați harta pentru a ajusta. În cele din urmă, faceți clic pe Salvare pentru a salva forma.
```
...
translated (by machine 🤖) as:
```
Tema de afișare (întunecată/luminoasă)
```
...
translated (by machine 🤖) as:
```
Doriți să încercați să importați un marcator din următoarele date text?
```
...
translated (by machine 🤖) as:
```
Selectați o opțiune pentru a salva fișierul exportat.
```
...
translated (by machine 🤖) as:
```
Fișierul exportat este mare și trebuie salvat cu „Salvare ca”.
```
...
translated (by machine 🤖) as:
```
Începe o nouă înregistrare a piesei
```
...
translated (by machine 🤖) as:
```
Înregistrare în curs de desfășurare
```
...
translated (by machine 🤖) as:
```
Doriți să finalizați și să salvați această piesă?
```
...
translated (by machine 🤖) as:
```
Doriți să acordați această permisiune?
```
...
translated (by machine 🤖) as:
```
Dacă doriți să acordați această permisiune, accesați setările de permisiuni ale acestei aplicații din setările sistemului.
```
...
translated (by machine 🤖) as:
```
Dacă lăsați această permisiune dezactivată, înregistrarea piesei va funcționa numai în timp ce aplicația este vizibilă în prim-plan.
```
...
translated (by machine 🤖) as:
```
Aplicația va trebui să afișeze o notificare pentru a rămâne activă în fundal în timp ce înregistrează o piesă.
```
...
translated (by machine 🤖) as:
```
Înregistrarea piesei (%s) a fost întreruptă înainte de a putea fi salvată. Vrei să-l salvezi acum?
```
...
translated (by machine 🤖) as:
```
Doriți să ștergeți pentru totdeauna piesele nesalvate?
```
...
translated (by machine 🤖) as:
```
Această pistă nu a avut puncte înregistrate și a fost pur și simplu aruncată.
```
...
translated (by machine 🤖) as:
```
Comutați punctele cu butoanele anterior/următorul, mutați harta pentru a plasa punctele și faceți clic pe „-” pentru a elimina un punct. În cele din urmă, faceți clic pe Salvare pentru a salva piesa editată.
```
...
translated (by machine 🤖) as:
```
%1$s ore %2$s minute %3$s secunde
```
...
translated (by machine 🤖) as:
```
Activați premium dintr-un cod generat de pe alt dispozitiv
```
...
translated (by machine 🤖) as:
```
Ați activat accesul premium pentru oaspeți pe acest dispozitiv
```
...
translated (by machine 🤖) as:
```
Generați un cod premium pentru a activa premium pe unul dintre celelalte dispozitive ale dvs
```
...
translated (by machine 🤖) as:
```
Cod premium generat cu succes: %s
Utilizați acest cod de pe celălalt dispozitiv pentru a activa premium acolo.
```
...
translated (by machine 🤖) as:
```
Creează o nouă hartă goală și aplică setări sigure pentru a preveni alte blocări
```
...
translated (by machine 🤖) as:
```
Configurarea sigură a fost aplicată și va fi efectivă la următoarea lansare, acum puteți să părăsiți aplicația și să o închideți.
```
...
translated (by machine 🤖) as:
```
Exportați datele de hartă disponibile local
```
...
translated (by machine 🤖) as:
```
Atenție: exportul de recuperare va fi parțial, cel puțin un fișier media cloud lipsește, nu va fi descărcat și va fi înlocuit cu un fișier gol în timpul exportului.
```
...
translated (by machine 🤖) as:
```
Dimensiunea de afișare a textului
```
...
translated (by machine 🤖) as:
```
Se pare că browserul dvs. nu este pe deplin compatibil - doar Google Chrome este testat ca fiind pe deplin compatibil
...
translated (by machine 🤖) as:
```
Acest lucru vă permite să vă sincronizați cu un server WebDAV prin configurarea acreditărilor de acces.
```
...
translated (by machine 🤖) as:
```
Introduceți valorile de configurare
```
...
translated (by machine 🤖) as:
```
Folosește vizualizarea nativă Google Maps
```
...
translated (by machine 🤖) as:
```
Acest mod de afișare ar putea fi mai lent, dar este mai puțin probabil să provoace blocări ale aplicației
```
...
translated (by machine 🤖) as:
```
(opțiunea curentă nu este aplicată, necesită o repornire completă a aplicației)
```
...
translated (by machine 🤖) as:
```
Aplicația va trebui să aibă permisiunea „întotdeauna” de acces la locație pentru a rămâne activă în fundal în timp ce înregistrează o pistă.
```
