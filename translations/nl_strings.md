...
translated (by machine 🤖) as:
```
Het lijkt erop dat uw browser niet volledig compatibel is - alleen Google Chrome is getest als volledig compatibel
...
translated (by machine 🤖) as:
```
Hiermee kunt u synchroniseren met een WebDAV-server door toegangsreferenties te configureren.
```
...
translated (by machine 🤖) as:
```
De app moet de locatietoestemming 'altijd' hebben om op de achtergrond actief te blijven tijdens het opnemen van een nummer.
```
