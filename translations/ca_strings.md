...
translated (by machine 🤖) as:
```
Afegiu un fitxer de mapa fora de línia
```
...
translated (by machine 🤖) as:
```
La importació de fitxers de mapes fora de línia s'ha realitzat correctament
```
...
translated (by machine 🤖) as:
```
Ha fallat la importació de fitxers de mapes fora de línia:
%s
```
...
translated (by machine 🤖) as:
```
Quin format voleu utilitzar per importar?
```
...
translated (by machine 🤖) as:
```
La sincronització s'ha cancel·lat perquè s'ha pogut trobar el fitxer següent: %s. Per solucionar-ho, podeu restaurar manualment el fitxer que falta o canviar la configuració de l'aplicació per no descarregar contingut multimèdia durant la sincronització.
```
...
translated (by machine 🤖) as:
```
Política de descàrrega de mitjans de sincronització
```
...
translated (by machine 🤖) as:
```
Baixeu tots els mitjans durant la sincronització
```
...
translated (by machine 🤖) as:
```
Baixeu contingut multimèdia després de la sincronització quan sigui necessari
```
...
translated (by machine 🤖) as:
```
Configuració de rendiment de renderització
```
...
translated (by machine 🤖) as:
```
Nombre màxim de títols de marcadors flotants dibuixats
```
...
translated (by machine 🤖) as:
```
Accediu a dades de mapes antigues
```
...
translated (by machine 🤖) as:
```
Consulteu les opcions per accedir a les dades del mapa de les versions anteriors de Map Marker.
```
...
translated (by machine 🤖) as:
```
S'estan migrant les dades del mapa...
```
...
translated (by machine 🤖) as:
```
La migració de dades de mapes ha estat correcta: ara hauríeu de poder utilitzar les vostres dades de mapes antigues en aquesta nova versió de Map Marker.
```
...
translated (by machine 🤖) as:
```
La migració de les dades del mapa ha fallat: no es pot accedir a les dades del mapa antigues en aquesta nova versió de Map Marker.
```
...
translated (by machine 🤖) as:
```
Això obrirà un enllaç del centre d'ajuda que explica com podeu instal·lar la versió antiga de Map Marker sense perdre les dades de l'aplicació.
```
...
translated (by machine 🤖) as:
```
Obriu i migra un mapa de núvols V2
```
...
translated (by machine 🤖) as:
```
Migreu un mapa de núvols heretat de Map Marker V2 (versió de l'aplicació d'abans del 2023) i obriu-lo com a mapa de núvols nou
```
...
translated (by machine 🤖) as:
```
S'està migrant el mapa de núvols heretat V2...
```
...
translated (by machine 🤖) as:
```
Seleccioneu el mapa V2 per migrar
```
...
translated (by machine 🤖) as:
```
La migració de mapes del núvol de Legacy V2 ha estat correcta!
```
...
translated (by machine 🤖) as:
```
Exporteu el marcador seleccionat com a fitxer
```
...
translated (by machine 🤖) as:
```
Voleu provar d'importar un marcador de les dades de text següents?
```
...
translated (by machine 🤖) as:
```
Sí, porta'm a la pàgina de l'aplicació
```
...
translated (by machine 🤖) as:
```
No, gràcies, no ho tornis a preguntar
```
...
translated (by machine 🤖) as:
```
Seleccioneu una opció per desar el fitxer exportat.
```
...
translated (by machine 🤖) as:
```
El fitxer exportat és gran i s'ha de desar amb "Desa com".
```
...
translated (by machine 🤖) as:
```
Enregistrament de la pista d'ubicació
```
...
translated (by machine 🤖) as:
```
Precisió de localització mínima
```
...
translated (by machine 🤖) as:
```
S'està iniciant l'enregistrament de la pista nova
```
...
translated (by machine 🤖) as:
```
Enregistrament de la pista en curs
```
...
translated (by machine 🤖) as:
```
Voleu acabar i desar aquesta pista?
```
...
translated (by machine 🤖) as:
```
Canvia els punts amb els botons anterior/següent, mou el mapa per col·locar els punts i fes clic a "-" per eliminar un punt. Finalment, feu clic a desa per desar la pista editada.
```
...
translated (by machine 🤖) as:
```
Velocitat de moviment mínima: %s
```
...
translated (by machine 🤖) as:
```
%1$s hores %2$s minuts %3$s segons
```
...
translated (by machine 🤖) as:
```
Introduïu el radi del cercle (%s)
```
...
translated (by machine 🤖) as:
```
Activa la premium a partir d'un codi generat des d'un altre dispositiu
```
...
translated (by machine 🤖) as:
```
No s'ha pogut activar la prima:
%s
```
...
translated (by machine 🤖) as:
```
Heu activat l'accés premium de convidat en aquest dispositiu
```
...
translated (by machine 🤖) as:
```
Genereu un codi premium per activar-lo en un dels vostres altres dispositius
```
...
translated (by machine 🤖) as:
```
Codi Premium generat correctament: %s
Utilitzeu aquest codi des del vostre altre dispositiu per activar-hi el premium.
```
...
translated (by machine 🤖) as:
```
Recuperació: configuració segura
```
...
translated (by machine 🤖) as:
```
Crea un mapa buit nou i aplica una configuració segura per evitar més bloquejos
```
...
translated (by machine 🤖) as:
```
S'ha aplicat la configuració segura i serà efectiva el proper llançament, ara podeu sortir de l'aplicació i finalitzar-la.
```
...
translated (by machine 🤖) as:
```
Recuperació: exportació de dades
```
...
translated (by machine 🤖) as:
```
Exporta les dades del mapa disponibles localment
```
...
translated (by machine 🤖) as:
```
Avís: l'exportació de recuperació serà parcial, falta almenys un fitxer multimèdia al núvol, no es baixarà i es substituirà per un fitxer buit durant l'exportació.
```
...
translated (by machine 🤖) as:
```
Mida de visualització del text
```
...
translated (by machine 🤖) as:
```
Utilitza la vista nativa Google Maps
```
...
translated (by machine 🤖) as:
```
Aquest mode de visualització pot ser més lent, però és menys probable que causi bloquejos de l'aplicació.
```
...
translated (by machine 🤖) as:
```
(l'opció actual no s'ha aplicat, cal reiniciar completament l'aplicació)
```
...
translated (by machine 🤖) as:
```
L'aplicació haurà de tenir el permís d'ubicació "sempre" per poder romandre activa en segon pla mentre grava una pista.
```
