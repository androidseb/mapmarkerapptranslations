...
translated (by machine 🤖) as:
```
للمتابعة ، تحتاج إلى تسجيل الدخول بحسابك السحابي. انقر فوق الزر "تسجيل الدخول" أدناه لتسجيل الدخول.
```
...
translated (by machine 🤖) as:
```
لا يمكن أن تكون قائمة الخيارات فارغة
```
...
translated (by machine 🤖) as:
```
الخيارات الفارغة غير مسموح بها
```
...
translated (by machine 🤖) as:
```
أتساءل عن كيفية تكوين مصادر البحث؟ انقر هنا.
```
...
translated (by machine 🤖) as:
```
نجح اختبار تكوين مصدر البحث! يجب أن يعمل البحث مع مصدر البحث هذا الآن.
```
...
translated (by machine 🤖) as:
```
تحذير: فشل اختبار تكوين مصدر البحث. قد لا يعمل البحث مع مصدر البحث هذا.
```
...
translated (by machine 🤖) as:
```
تجربة البحث الافتراضية ليست الأمثل ، ولكنها تعمل دون تكوين.
...
انقر فوق الزر "تغيير المصدر" ثم خيار "البرنامج التعليمي" لمعرفة المزيد.
```
...
translated (by machine 🤖) as:
```
تم إحباط المزامنة لأنه تم العثور على الملف التالي: %s. لحل هذه المشكلة ، يمكنك إما استعادة الملف المفقود يدويًا أو تغيير إعدادات التطبيق لعدم تنزيل الوسائط أثناء المزامنة.
```
...
translated (by machine 🤖) as:
```
قم بتنزيل جميع الوسائط أثناء المزامنة
```
...
translated (by machine 🤖) as:
```
الوصول إلى بيانات الخرائط القديمة
```
...
translated (by machine 🤖) as:
```
اطلع على خيارات الوصول إلى بيانات الخريطة من الإصدارات الأقدم من Map Marker.
```
...
translated (by machine 🤖) as:
```
نجح ترحيل بيانات الخريطة: يجب أن تكون الآن قادرًا على استخدام بيانات الخريطة القديمة في هذا الإصدار الجديد من Map Marker.
```
...
translated (by machine 🤖) as:
```
فشل ترحيل بيانات الخريطة: الوصول إلى بيانات الخريطة القديمة غير ممكن في هذا الإصدار الجديد من Map Marker.
```
...
translated (by machine 🤖) as:
```
لاحظ أن بيانات الخريطة القديمة تظل دون تغيير وستظل على جهازك حتى تقوم بإلغاء تثبيت التطبيق. سيؤدي إلغاء تثبيت التطبيق إلى حذف جميع بيانات التطبيق Map Marker من جهازك. إذا كنت ترغب في استخدام الإصدار القديم من Map Marker ، يمكنك اتباع الإرشادات المتوفرة بالنقر فوق الخيار أدناه.
```
...
translated (by machine 🤖) as:
```
سيؤدي هذا إلى فتح رابط مركز المساعدة يشرح كيف يمكنك تثبيت الإصدار القديم من Map Marker دون فقد بيانات التطبيق.
```
...
translated (by machine 🤖) as:
```
قم بترحيل خريطة سحابة قديمة من Map Marker V2 (إصدار التطبيق قبل عام 2023) وافتحها كخريطة سحابية جديدة
```
...
translated (by machine 🤖) as:
```
ترحيل خريطة السحابة القديمة V2 ...
```
...
translated (by machine 🤖) as:
```
نجح ترحيل الخرائط السحابية القديمة V2!
```
...
translated (by machine 🤖) as:
```
فشل ترحيل الخرائط السحابية القديمة V2:
%s
```
...
translated (by machine 🤖) as:
```
تم الحفظ بنجاح في ملفات الجهاز
```
...
translated (by machine 🤖) as:
```
كيف تريد استخدام الملف الذي تم تصديره؟
```
...
translated (by machine 🤖) as:
```
هل تريد محاولة استيراد علامة من البيانات النصية التالية؟
```
...
translated (by machine 🤖) as:
```
حدد خيارًا لحفظ الملف الذي تم تصديره.
```
...
translated (by machine 🤖) as:
```
الملف الذي تم تصديره كبير ويجب حفظه باستخدام "حفظ باسم".
```
...
translated (by machine 🤖) as:
```
هل تريد إنهاء هذا المسار وحفظه؟
```
...
translated (by machine 🤖) as:
```
إذا كنت تريد منح هذا الإذن، فانتقل إلى إعدادات أذونات هذا التطبيق ضمن إعدادات النظام لديك.
```
...
translated (by machine 🤖) as:
```
إذا تركت هذا الإذن معطلاً، فلن يعمل تسجيل المسار إلا عندما يكون التطبيق مرئيًا في المقدمة.
```
...
translated (by machine 🤖) as:
```
سيحتاج التطبيق إلى عرض إشعار ليظل نشطًا في الخلفية أثناء تسجيل المسار.
```
...
translated (by machine 🤖) as:
```
تمت مقاطعة تسجيل المسار (المسارات) (%s) قبل أن يتم حفظه. هل تريد حفظه الآن؟
```
...
translated (by machine 🤖) as:
```
هل تريد حذف المسار (المسارات) غير المحفوظ الذي تم تسجيله إلى الأبد؟
```
...
translated (by machine 🤖) as:
```
لم يتم تسجيل أي نقاط في هذا المسار وتم تجاهله ببساطة.
```
...
translated (by machine 🤖) as:
```
يجمع هذا التطبيق بيانات الموقع لتمكين ميزة "تسجيل المسارات" حتى عندما يكون التطبيق مغلقًا أو غير مستخدم.

يتم استخدام جمع بيانات الموقع هذا حصريًا لتسجيل المسارات في العلامات الخاصة بك، ويحدث فقط عندما تبدأ تسجيل المسار بشكل صريح.

يمكنك العثور على مزيد من المعلومات العامة حول الخصوصية في سياسة الخصوصية المتوفرة على الرابط التالي:
%s
```
...
translated (by machine 🤖) as:
```
قم بتبديل النقاط باستخدام الأزرار السابقة/التالي، وحرك الخريطة لوضع النقاط وانقر فوق "-" لإزالة نقطة. وأخيرا انقر فوق حفظ لحفظ المسار المحرر.
```
...
translated (by machine 🤖) as:
```
قم بتفعيل قسط التأمين من خلال رمز تم إنشاؤه من جهاز آخر
```
...
translated (by machine 🤖) as:
```
لقد قمت بتنشيط وصول الضيف المميز على هذا الجهاز
```
...
translated (by machine 🤖) as:
```
أنشئ رمزًا مميزًا لتفعيل الاشتراك المميز على أحد أجهزتك الأخرى
```
...
translated (by machine 🤖) as:
```
تم إنشاء الرمز المميز بنجاح: %s
استخدم هذا الرمز من جهازك الآخر لتفعيل الإصدار المميز هناك.
```
...
translated (by machine 🤖) as:
```
إنشاء خريطة فارغة جديدة وتطبيق الإعدادات الآمنة لمنع المزيد من الأعطال
```
...
translated (by machine 🤖) as:
```
تم تطبيق الإعداد الآمن وسيصبح فعالاً عند التشغيل التالي، ويمكنك الآن الخروج من التطبيق وإنهائه.
```
...
translated (by machine 🤖) as:
```
تصدير بيانات الخرائط المتوفرة محليًا
```
...
translated (by machine 🤖) as:
```
تحذير: سيكون تصدير الاسترداد جزئيًا، وهناك ملف وسائط سحابي واحد على الأقل مفقود، ولن يتم تنزيله وسيتم استبداله بملف فارغ أثناء التصدير.
```
...
translated (by machine 🤖) as:
```
يبدو أن متصفحك غير متوافق تمامًا - تم اختبار Google Chrome فقط باعتباره متوافقًا تمامًا
...
translated (by machine 🤖) as:
```
إظهار تذكير النسخ الاحتياطي للبيانات
```
...
translated (by machine 🤖) as:
```
عرض جميع نقاط شكل التحرير (أبطأ)
```
...
translated (by machine 🤖) as:
```
يتيح لك هذا المزامنة مع خادم WebDAV عن طريق تكوين بيانات اعتماد الوصول.
```
...
translated (by machine 🤖) as:
```
يتم الاتصال بحسابك Google عبر خطوتين: المصادقة والتفويض.
...
translated (by machine 🤖) as:
```
في انتظار تحديد الموقع عبر نظام تحديد المواقع العالمي (GPS).
```
...
translated (by machine 🤖) as:
```
يتوقف تسجيل المسار على دقة موقع GPS كافية. يمكنك ضبط مستوى الدقة من إعدادات التطبيق.
```
...
translated (by machine 🤖) as:
```
فشل فتح النافذة المنبثقة، على الأرجح تم حظرها بواسطة متصفحك، تحقق من إعدادات متصفحك للسماح بالنوافذ المنبثقة على هذا الموقع، تحتوي معظم المتصفحات على زر "السماح" يظهر حول شريط العنوان عند حظر النوافذ المنبثقة.
```
