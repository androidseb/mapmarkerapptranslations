...
translated (by human 👀) as:
```
Ceci donne à l'application un accès complet à votre Google Drive. Choisissez cette option si vous prévoyez de collaborer avec d'autres comptes Google.
```
...
translated (by human 👀) as:
```
Limite de volume sur données mobiles
```
...
translated (by human 👀) as:
```
Ceci limite l'accès de l'application au dossier dédié à l'application par Dropbox. Dans le doute, choisissez cette option, vous pouvez facilement changer d'avis plus tard.
```
...
translated (by human 👀) as:
```
Vider les fichiers de carte hors ligne
```
...
translated (by human 👀) as:
```
Écrivez plus puis pour raffiner votre recherche
```
...
translated (by human 👀) as:
```
Vous pouvez chercher des endroits avec "%s". Cette recherche nécessite une connexion internet.
```
...
translated (by human 👀) as:
```
Accéder aux anciennes données de carte
```
...
translated (by human 👀) as:
```
Voir les options pour accéder aux données de carte d'anciennes versions de Map Marker.
```
...
translated (by human 👀) as:
```
Migration des données de carte…
```
...
translated (by human 👀) as:
```
Migration des données de carte réussie: vous devriez désormais pouvoir utiliser vos anciennes données de carte sur cette nouvelle version de Map Marker.
```
...
translated (by human 👀) as:
```
Migration des données de carte échouée: l'accès à vos anciennes données de carte n'est pas possible sur cette nouvelle version de Map Marker.
```
...
translated (by human 👀) as:
```
Ceci ouvrira un lien vers le centre d'aide expliquant comment vous pouvez installer l'ancienne version de Map Marker sans perdre vos données d'application.
```
...
translated (by human 👀) as:
```
La dernière sauvegarde date de %s jours
```
...
translated (by human 👀) as:
```
Ouvrir et migrer une carte cloud V2
```
...
translated (by human 👀) as:
```
Exporter le marqueur sélectionné en fichier
```
...
translated (by human 👀) as:
```
Comment voulez-vous utiliser le fichier exporté ?
```
...
translated (by human 👀) as:
```
Sélectionnez le centre ou le bord du cercle avec les boutons de la barre d'outils et déplacez la carte pour ajuster. Enfin, cliquez sauvegarder pour enregistrer la forme.
```
...
translated (by machine 🤖) as:
```
Thème d'affichage (sombre / clair)
```
...
translated (by machine 🤖) as:
```
Voulez-vous essayer d'importer un marqueur à partir des données texte suivantes ?
```
...
translated (by machine 🤖) as:
```
Oui, dirigez-moi vers la page de l'application
```
...
translated (by machine 🤖) as:
```
Peut-être plus tard, rappellez-moi
```
...
translated (by machine 🤖) as:
```
Sélectionnez une option pour enregistrer le fichier exporté.
```
...
translated (by machine 🤖) as:
```
Le fichier exporté est volumineux et doit être enregistré avec 'Enregistrer sous'.
```
...
translated (by machine 🤖) as:
```
Intervalle de distance minimum
```
...
translated (by machine 🤖) as:
```
Démarrage d'un nouveau tracé de localisation
```
...
translated (by machine 🤖) as:
```
Tracé de localisation en cours
```
...
translated (by machine 🤖) as:
```
Voulez-vous terminer et sauvegarder ce tracé ?
```
...
translated (by machine 🤖) as:
```
Voulez-vous donner cette permission ?
```
...
translated (by machine 🤖) as:
```
Si vous voulez donner cette permission, ouvrez les réglages de permissions de l'application dans les réglages système.
```
...
translated (by machine 🤖) as:
```
Si vous laissez cette permission désactivée, l'enregistrement de tracé fonctionneral seulement lorsque l'application est visible en avant-plan.
```
...
translated (by machine 🤖) as:
```
L'application aura besoin d'afficher une notification afin de rester active en arrière plan durant l'enregistrement d'un tracé.
```
...
translated (by machine 🤖) as:
```
L'enregistrement de tracé(s) (%s) a été interrompu avant de pouvoir être sauvegardé. Voulez-vous le sauvegarder maintenant ?
```
...
translated (by machine 🤖) as:
```
Voulez-vous supprimer l'enregistrement de tracé non-sauvegardé pour toujours ?
```
...
translated (by machine 🤖) as:
```
Ce tracé n'a enregistré aucun point et a simplement été annulé.
```
...
translated (by machine 🤖) as:
```
Voir des informations sur la confidentialité
```
...
translated (by machine 🤖) as:
```
Cette application collecte des données de localisation pour permettre la fonctionnalité 'enregistrement de tracés' même lorsque l'application est fermée ou n'est pas utilisée.

Cette collecte de données de localisation est utilisée exclusivement pour enregistrer des tracés dans vos marqueurs, et ne se produit que lorsque vous avez explicitement lancé un enregistrement de tracé.

Vous pouvez trouver plus d'information sur la confidentialité dans la politique de confidentialité disponible au lien suivant:
%s
```
...
translated (by machine 🤖) as:
```
Changez de point avec les boutons suivant/précédent, déplacez la carte pour placer les points, et cliquez '-' pour retirer un point. Enfin, cliquez sauvegarder pour enregistrer le tracé.
```
...
translated (by machine 🤖) as:
```
Vitesse minimum en déplacement: %s
```
...
translated (by machine 🤖) as:
```
%1$s heures %2$s minutes %3$s secondes
```
...
translated (by machine 🤖) as:
```
Activer l'accès premium sur cet appareil avec une code généré depuis un autre appareil
```
...
translated (by machine 🤖) as:
```
Vous avez activé l'accès premium invité sur cet appareil
```
...
translated (by machine 🤖) as:
```
Générer un code premium à activer sur un de vos autres appareils
```
...
translated (by machine 🤖) as:
```
Code premium généré avec succès: %s
Utilisez ce code depuis votre autre appareil pour y activer l'accès premium.
```
...
translated (by machine 🤖) as:
```
Créé une nouvelle carte vide et applique des réglages stables pour éviter des crashs supplémentaires
```
...
translated (by machine 🤖) as:
```
Les réglages stables ont été appliqués et seront effectifs lors du prochain lancement, vous pouvez maintenant quitter l'application et la terminer.
```
...
translated (by machine 🤖) as:
```
Exporter les données de carte disponibles localement
```
...
translated (by machine 🤖) as:
```
Attention: l'exportation de récupération sera partielle, au moins un fichier média cloud est manquant, ne sera pas téléchargé et sera remplacé avec un fichier vide.
```
...
translated (by machine 🤖) as:
```
Il semble que votre navigateur ne soit pas entièrement compatible - seul Google Chrome est testé comme pleinement compatible.
...
translated (by machine 🤖) as:
```
Afficher le rappel de sauvegarde de données
```
...
translated (by machine 🤖) as:
```
L'application aura besoin de l'accès 'toujours' à la position de l'appareil afin de rester active en arrière plan durant l'enregistrement d'un tracé.
```
...
translated (by machine 🤖) as:
```
Mètre carré (m²) / Hectare (ha) / Kilomètre carré (km²)
```
...
translated (by machine 🤖) as:
```
Pied carré (ft²) / Acre (ac) / Mile carré (mi²)
```
