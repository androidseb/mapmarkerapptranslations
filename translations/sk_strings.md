...
translated (by machine 🤖) as:
```
Presunúť priečinky do priečinka
```
...
translated (by machine 🤖) as:
```
Zadajte svoje ID prístupového kľúča
```
...
translated (by machine 🤖) as:
```
Zadajte svoj tajný prístupový kľúč
```
...
translated (by machine 🤖) as:
```
Vždy zobraziť predvolený priečinok
```
...
translated (by machine 🤖) as:
```
V tomto mapovom súbore nemôžete robiť zmeny, pretože je v režime iba na čítanie.
```
...
translated (by machine 🤖) as:
```
Vyberte offline mapový súbor, ktorý chcete skopírovať do priečinka offline máp: %s
```
...
translated (by machine 🤖) as:
```
Importuje sa offline súbor mapy ...
```
...
translated (by machine 🤖) as:
```
Ďalšia veľká aktualizácia na Map Marker príde čoskoro! Táto nová verzia prinesie dôležité vylepšenia a nové funkcie.

Aktualizácia aplikácie zostane bezplatná a voliteľná, kliknutím na „Zobraziť viac“ získate ďalšie informácie.
```
...
translated (by machine 🤖) as:
```
V cloude sa nenašiel žiadny priečinok s mapami – ak hľadáte staré cloudové mapy vytvorené pomocou starej verzie Map Marker (V2), skúste namiesto toho možnosť „Otvoriť a migrovať cloudovú mapu V2“.
```
