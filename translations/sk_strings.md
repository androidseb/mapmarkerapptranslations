...
translated (by machine 🤖) as:
```
Priečinok nemôžete presunúť k niektorému z jeho potomkov.
```
...
translated (by machine 🤖) as:
```
Momentálne nie je nakonfigurovaný žiadny zdroj vyhľadávania. Aby ste mohli vyhľadávať, musíte nakonfigurovať vyhľadávací zdroj tretej strany.

Malo by to trvať len trochu času a umožní vám bezplatný prístup k výsledkom vyhľadávania.

Kliknite na tlačidlo „Zmeniť zdroj“ a potom na možnosť „Výukový program“, kde sa dozviete viac.
```
...
translated (by machine 🤖) as:
```
Import súborov offline mapy bol úspešný
```
...
translated (by machine 🤖) as:
```
Import súboru offline mapy sa nepodaril:
%s
```
...
translated (by machine 🤖) as:
```
Veľkosť špendlíka základnej mapy
```
...
translated (by machine 🤖) as:
```
Aký formát chcete použiť na import?
```
...
translated (by machine 🤖) as:
```
Ak chcete spresniť vyhľadávanie, zadajte viac
```
...
translated (by machine 🤖) as:
```
Miesta môžete vyhľadávať pomocou „%s“. Toto vyhľadávanie vyžaduje internetové pripojenie.
```
...
translated (by machine 🤖) as:
```
Synchronizácia bola prerušená, pretože ste mohli nájsť nasledujúci súbor: %s. Ak to chcete vyriešiť, môžete buď obnoviť chýbajúci súbor manuálne, alebo zmeniť nastavenia aplikácie tak, aby sa počas synchronizácie nesťahovali médiá.
```
...
translated (by machine 🤖) as:
```
Synchronizovať pravidlá sťahovania médií
```
...
translated (by machine 🤖) as:
```
Stiahnite si všetky médiá počas synchronizácie
```
...
translated (by machine 🤖) as:
```
V prípade potreby stiahnite médiá po synchronizácii
```
...
translated (by machine 🤖) as:
```
Po zistení zmien sa automaticky synchronizuje s cloudom
```
...
translated (by machine 🤖) as:
```
Vyberte atribúty, ktoré je potrebné zvážiť pri odstraňovaní duplikátov. Ak sú vybraté všetky atribúty, odstránia sa iba striktne identické duplicitné položky. Ak nie sú vybraté žiadne atribúty, duplicitné položky s rovnakým umiestnením budú odstránené bez ohľadu na ich ostatné atribúty.
```
...
translated (by machine 🤖) as:
```
Našlo sa %d duplikátov. Naozaj chcete odstrániť tieto duplikáty?
```
...
translated (by machine 🤖) as:
```
Nastavenia výkonu vykresľovania
```
...
translated (by machine 🤖) as:
```
Maximálny počet vykreslených bodov záujmu
```
...
translated (by machine 🤖) as:
```
Maximálny počet vyžrebovaných plávajúcich názvov značiek
```
...
translated (by machine 🤖) as:
```
Prístup k starým mapovým údajom
```
...
translated (by machine 🤖) as:
```
Pozrite si možnosti prístupu k údajom mapy zo starších verzií Map Marker.
```
...
translated (by machine 🤖) as:
```
Migrácia údajov máp bola úspešná: teraz by ste mali môcť používať svoje staré údaje máp v tejto novej verzii Map Marker.
```
...
translated (by machine 🤖) as:
```
Migrácia údajov máp zlyhala: v tejto novej verzii Map Marker nie je možný prístup k vašim starým údajom máp.
```
...
translated (by machine 🤖) as:
```
Upozorňujeme, že staré údaje mapy zostanú nezmenené a zostanú vo vašom zariadení, kým aplikáciu neodinštalujete. Odinštalovaním aplikácie sa z vášho zariadenia odstránia všetky údaje aplikácie Map Marker. Ak chcete použiť starú verziu Map Marker, môžete postupovať podľa pokynov dostupných po kliknutí na možnosť nižšie.
```
...
translated (by machine 🤖) as:
```
Posledná záloha bola vykonaná pred %s dňami
```
...
translated (by machine 🤖) as:
```
Znova spustite migráciu údajov
```
...
translated (by machine 🤖) as:
```
Vyberte starú mapu, na ktorej znova spustíte migráciu údajov
```
...
translated (by machine 🤖) as:
```
Opätovná migrácia údajov máp bola úspešná!
```
...
translated (by machine 🤖) as:
```
Opätovná migrácia údajov máp zlyhala:
%s
```
...
translated (by machine 🤖) as:
```
Otvorte a migrujte cloudovú mapu V2
```
...
translated (by machine 🤖) as:
```
Migrujte starú cloudovú mapu z Map Marker V2 (verzia aplikácie pred rokom 2023) a otvorte ju ako novú cloudovú mapu
```
...
translated (by machine 🤖) as:
```
Migrácia staršej cloudovej mapy V2 bola úspešná!
```
...
translated (by machine 🤖) as:
```
Exportujte vybratú značku ako súbor
```
...
translated (by machine 🤖) as:
```
Pomocou tlačidiel na paneli s nástrojmi vyberte stred alebo okraj kruhu a posúvaním mapy upravte. Nakoniec kliknutím na tlačidlo uložiť tvar uložte.
```
...
translated (by machine 🤖) as:
```
Chcete sa pokúsiť importovať značku z nasledujúcich textových údajov?
```
...
translated (by machine 🤖) as:
```
Áno, prejsť na stránku aplikácie
```
...
translated (by machine 🤖) as:
```
Vyberte možnosť uloženia exportovaného súboru.
```
...
translated (by machine 🤖) as:
```
Exportovaný súbor je veľký a je potrebné ho uložiť pomocou možnosti „Uložiť ako“.
```
...
translated (by machine 🤖) as:
```
Minimálny interval vzdialenosti
```
...
translated (by machine 🤖) as:
```
Spustenie nahrávania novej trasy
```
...
translated (by machine 🤖) as:
```
Chcete dokončiť a uložiť túto trasu?
```
...
translated (by machine 🤖) as:
```
Ak chcete udeliť toto povolenie, prejdite do nastavení povolení tejto aplikácie v nastaveniach systému.
```
...
translated (by machine 🤖) as:
```
Ak toto povolenie necháte vypnuté, nahrávanie trasy bude fungovať len vtedy, keď je aplikácia viditeľná v popredí.
```
...
translated (by machine 🤖) as:
```
Aplikácia bude musieť zobraziť upozornenie, aby zostala aktívna na pozadí počas nahrávania trasy.
```
...
translated (by machine 🤖) as:
```
Nahrávanie stôp (%s) bolo prerušené skôr, ako sa dalo uložiť. Chcete to teraz uložiť?
```
...
translated (by machine 🤖) as:
```
Chcete navždy vymazať neuložené záznamy stôp?
```
...
translated (by machine 🤖) as:
```
Táto skladba nemala zaznamenané žiadne body a bola jednoducho vyradená.
```
...
translated (by machine 🤖) as:
```
%1$s hodiny %2$s minúty %3$s sekúnd
```
...
translated (by machine 🤖) as:
```
Aktivujte prémium z kódu vygenerovaného z iného zariadenia
```
...
translated (by machine 🤖) as:
```
Nepodarilo sa aktivovať prémiu:
%s
```
...
translated (by machine 🤖) as:
```
Na tomto zariadení ste aktivovali prémiový prístup pre hostí
```
...
translated (by machine 🤖) as:
```
Vygenerujte prémiový kód na aktiváciu prémie na jednom z vašich ďalších zariadení
```
...
translated (by machine 🤖) as:
```
Prémiový kód bol úspešne vygenerovaný: %s
Použite tento kód zo svojho druhého zariadenia na aktiváciu prémie tam.
```
...
translated (by machine 🤖) as:
```
Zdá sa, že váš prehliadač nie je plne kompatibilný – ako plne kompatibilný je testovaný iba prehliadač Google Chrome
Podrobnosti o kompatibilite nájdete nižšie:
...
translated (by machine 🤖) as:
```
Zobraziť pripomenutie zálohovania údajov
```
...
translated (by machine 🤖) as:
```
Zobraziť všetky body tvaru pri úprave (pomalšie)
```
...
translated (by machine 🤖) as:
```
To vám umožní synchronizáciu so serverom WebDAV pomocou konfigurácie prístupových poverení.
```
...
translated (by machine 🤖) as:
```
Aplikácia bude potrebovať povolenie na prístup k polohe „vždy“, aby mohla počas zaznamenávania trasy zostať aktívna na pozadí.
```
...
translated (by machine 🤖) as:
```
Štvorcový meter (m²) / Hektár (ha) / Štvorcový kilometer (km²)
```
...
translated (by machine 🤖) as:
```
Štvorcová stopa (ft²) / Aker (ac) / Štvorcová míľa (mi²)
```
...
translated (by machine 🤖) as:
```
Pripojenie k vášmu účtu Google sa vykonáva v dvoch krokoch: overenie a autorizácia.
Počas procesu pripájania sa môžu zobraziť dve kontextové okná za sebou – v takom prípade sa uistite, že ste oba razy vybrali rovnaký účet Google.

%1$s Krok 1/2 – overenie

%2$s Krok 2/2 – autorizácia
```
...
translated (by machine 🤖) as:
```
Vyskytla sa neočakávaná chyba overenia Google. Zdá sa, že lokálna vyrovnávacia pamäť vášho zariadenia je poškodená.
Môže sa to stať, ak ste nedávno zrušili / obnovili / odstránili pripojenie medzi touto aplikáciou a vaším účtom Google.
Vyrovnávacia pamäť zariadenia by sa mala automaticky obnovovať každú hodinu.
Počkajte jednu alebo dve hodiny a skúste to znova.
```
