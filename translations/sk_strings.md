...
translated (by machine 🤖) as:
```
Použiť natívne zobrazenie Google Maps
```
...
translated (by machine 🤖) as:
```
Tento režim zobrazenia môže byť pomalší, ale je menej pravdepodobné, že spôsobí zlyhanie aplikácie
```
...
translated (by machine 🤖) as:
```
(aktuálna voľba sa nepoužila, vyžaduje sa úplný reštart aplikácie)
```
