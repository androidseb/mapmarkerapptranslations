...
translated (by machine 🤖) as:
```
Přidejte offline mapový soubor
```
...
translated (by machine 🤖) as:
```
Import souboru offline mapy byl úspěšný
```
...
translated (by machine 🤖) as:
```
Import souboru offline mapy se nezdařil:
%s
```
...
translated (by machine 🤖) as:
```
Velikost špendlíku základní mapy
```
...
translated (by machine 🤖) as:
```
Jaký formát chcete použít pro import?
```
...
translated (by machine 🤖) as:
```
Synchronizace byla přerušena, protože byl nalezen následující soubor: %s. Chcete-li to vyřešit, můžete buď obnovit chybějící soubor ručně, nebo změnit nastavení aplikace tak, aby během synchronizace nestahovala média.
```
...
translated (by machine 🤖) as:
```
Zásady stahování médií synchronizace
```
...
translated (by machine 🤖) as:
```
Stáhnout všechna média během synchronizace
```
...
translated (by machine 🤖) as:
```
V případě potřeby stáhněte média po synchronizaci
```
...
translated (by machine 🤖) as:
```
Maximální počet vylosovaných POI
```
...
translated (by machine 🤖) as:
```
Maximální počet vylosovaných titulů plovoucích značek
```
...
translated (by machine 🤖) as:
```
Přístup ke starým mapovým datům
```
...
translated (by machine 🤖) as:
```
Podívejte se na možnosti přístupu k mapovým datům ze starších verzí Map Marker.
```
...
translated (by machine 🤖) as:
```
Migrace mapových dat byla úspěšná: nyní byste měli být schopni používat svá stará mapová data v této nové verzi Map Marker.
```
...
translated (by machine 🤖) as:
```
Migrace mapových dat se nezdařila: přístup k vašim starým mapovým datům není v této nové verzi Map Marker možný.
```
...
translated (by machine 🤖) as:
```
Upozorňujeme, že stará mapová data zůstanou nezměněna a zůstanou ve vašem zařízení, dokud aplikaci neodinstalujete. Odinstalováním aplikace smažete ze zařízení všech Map Marker dat aplikace. Pokud chcete používat starou verzi Map Marker, můžete se řídit pokyny dostupnými po kliknutí na možnost níže.
```
...
translated (by machine 🤖) as:
```
Otevře se odkaz na centrum nápovědy s vysvětlením, jak nainstalovat starou verzi Map Marker, aniž byste přišli o data aplikace.
```
...
translated (by machine 🤖) as:
```
Poslední záloha byla provedena před %s dny
```
...
translated (by machine 🤖) as:
```
Migrace starší cloudové mapy V2…
```
...
translated (by machine 🤖) as:
```
Vyberte mapu V2, kterou chcete migrovat
```
...
translated (by machine 🤖) as:
```
Migrace starší cloudové mapy V2 byla úspěšná!
```
...
translated (by machine 🤖) as:
```
Migrace starší cloudové mapy V2 se nezdařila:
%s
```
...
translated (by machine 🤖) as:
```
Jak chcete použít exportovaný soubor?
```
...
translated (by machine 🤖) as:
```
Vyberte střed nebo okraj kruhu pomocí tlačítek na panelu nástrojů a pohybem mapy upravte. Nakonec kliknutím na uložit tvar uložte.
```
...
translated (by machine 🤖) as:
```
Chcete zkusit importovat značku z následujících textových dat?
```
...
translated (by machine 🤖) as:
```
Ano, přejít na stránku aplikace
```
...
translated (by machine 🤖) as:
```
Možná později, připomeňte mi to
```
...
translated (by machine 🤖) as:
```
Vyberte možnost pro uložení exportovaného souboru.
```
...
translated (by machine 🤖) as:
```
Exportovaný soubor je velký a musí být uložen pomocí 'Uložit jako'.
```
...
translated (by machine 🤖) as:
```
Minimální interval vzdálenosti
```
...
translated (by machine 🤖) as:
```
Chcete dokončit a uložit tuto trasu?
```
...
translated (by machine 🤖) as:
```
Přepínejte body pomocí tlačítek předchozí/další, pohybujte mapou a umístěte body a kliknutím na '-' bod odstraňte. Nakonec klikněte na Uložit pro uložení upravené stopy.
```
...
translated (by machine 🤖) as:
```
%1$s hodiny %2$s minuty %3$s sekund
```
...
translated (by machine 🤖) as:
```
Aktivujte prémii z kódu vygenerovaného z jiného zařízení
```
...
translated (by machine 🤖) as:
```
Nepodařilo se aktivovat prémii:
%s
```
...
translated (by machine 🤖) as:
```
Na tomto zařízení jste aktivovali prémiový přístup pro hosty
```
...
translated (by machine 🤖) as:
```
Vygenerujte prémiový kód pro aktivaci prémie na jednom ze svých dalších zařízení
```
...
translated (by machine 🤖) as:
```
Prémiový kód byl úspěšně vygenerován: %s
Použijte tento kód ze svého druhého zařízení k aktivaci prémie tam.
```
...
translated (by machine 🤖) as:
```
Vytvoří novou prázdnou mapu a použije bezpečná nastavení, aby se zabránilo dalším haváriím
```
...
translated (by machine 🤖) as:
```
Bezpečné nastavení bylo použito a bude účinné při příštím spuštění. Nyní můžete aplikaci ukončit a ukončit ji.
```
...
translated (by machine 🤖) as:
```
Exportujte lokálně dostupná mapová data
```
...
translated (by machine 🤖) as:
```
Upozornění: Export pro obnovení bude částečný, chybí alespoň jeden soubor cloudového média, nebude stažen a během exportu bude nahrazen prázdným souborem.
```
...
translated (by machine 🤖) as:
```
Zdá se, že váš prohlížeč není plně kompatibilní – jako plně kompatibilní je testován pouze Google Chrome
...
translated (by machine 🤖) as:
```
To vám umožní synchronizovat se serverem WebDAV nakonfigurováním přístupových pověření.
```
...
translated (by machine 🤖) as:
```
Použít nativní zobrazení Google Maps
```
...
translated (by machine 🤖) as:
```
Tento režim zobrazení může být pomalejší, ale je méně pravděpodobné, že způsobí pády aplikace.
```
...
translated (by machine 🤖) as:
```
(aktuální volba se nepoužívá, vyžaduje úplný restart aplikace)
```
...
translated (by machine 🤖) as:
```
Aplikace bude potřebovat oprávnění „vždy“ k přístupu k poloze, aby mohla zůstat aktivní na pozadí během nahrávání trasy.
```
