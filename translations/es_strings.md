...
translated (by machine 🤖) as:
```
Si desea otorgar este permiso, vaya a la configuración de permisos de esta aplicación en la configuración de su sistema.
```
...
translated (by machine 🤖) as:
```
Si deja este permiso deshabilitado, la grabación de pistas solo funcionará mientras la aplicación esté visible en primer plano.
```
...
translated (by machine 🤖) as:
```
La aplicación deberá mostrar una notificación para permanecer activa en segundo plano mientras graba una pista.
```
...
translated (by machine 🤖) as:
```
La grabación de la(s) pista(s) (%s) se interrumpió antes de poder guardarla. ¿Quieres guardarlo ahora?
```
...
translated (by machine 🤖) as:
```
¿Quieres eliminar para siempre la grabación de las pistas no guardadas?
```
...
translated (by machine 🤖) as:
```
Esta pista no tenía puntos registrados y simplemente ha sido descartada.
```
...
translated (by machine 🤖) as:
```
Ver información sobre privacidad
```
...
translated (by machine 🤖) as:
```
Esta aplicación recopila datos de ubicación para habilitar la función de "grabación de pistas" incluso cuando la aplicación está cerrada o no está en uso.

Esta recopilación de datos de ubicación se utiliza exclusivamente para grabar recorridos en sus marcadores y solo ocurre cuando usted ha iniciado explícitamente una grabación de recorrido.

Puede encontrar más información general sobre privacidad en la política de privacidad disponible en el siguiente enlace:
%s
```
...
translated (by machine 🤖) as:
```
Cambie de punto con los botones anterior/siguiente, mueva el mapa para colocar los puntos y haga clic en '-' para eliminar un punto. Finalmente haga clic en guardar para guardar la pista editada.
```
...
translated (by machine 🤖) as:
```
Velocidad mínima de movimiento: %s
```
...
translated (by machine 🤖) as:
```
%1$s horas %2$s minutos %3$s segundos
```
...
translated (by machine 🤖) as:
```
Introduzca el radio del círculo (%s)
```
...
translated (by machine 🤖) as:
```
Activar la prima a partir de un código generado desde otro dispositivo
```
...
translated (by machine 🤖) as:
```
Has activado el acceso premium para invitados en este dispositivo
```
...
translated (by machine 🤖) as:
```
Genera un código premium para activar la prima en uno de tus otros dispositivos
```
...
translated (by machine 🤖) as:
```
Código premium generado exitosamente: %s
Utilice este código desde su otro dispositivo para activar la prima allí.
```
...
translated (by machine 🤖) as:
```
Recuperación: configuración segura
```
...
translated (by machine 🤖) as:
```
Crea un nuevo mapa vacío y aplica configuraciones seguras para evitar más fallas
```
...
translated (by machine 🤖) as:
```
La configuración segura se aplicó y será efectiva en el próximo inicio. Ahora puede salir de la aplicación y finalizarla.
```
...
translated (by machine 🤖) as:
```
Exportar datos de mapas disponibles localmente
```
...
translated (by machine 🤖) as:
```
Advertencia: la exportación de recuperación será parcial, falta al menos un archivo multimedia en la nube, no se descargará y será reemplazado por un archivo vacío durante la exportación.
```
...
translated (by machine 🤖) as:
```
Tamaño de visualización del texto
```
...
translated (by machine 🤖) as:
```
Parece que su navegador no es totalmente compatible. Solo Google Chrome se ha probado como totalmente compatible.
Vea los detalles de compatibilidad a continuación:
...
translated (by machine 🤖) as:
```
Mostrar recordatorio de copia de seguridad de datos
```
...
translated (by machine 🤖) as:
```
Mostrar todos los puntos de forma de edición (más lento)
```
...
translated (by machine 🤖) as:
```
Esto le permite sincronizar con un servidor WebDAV configurando las credenciales de acceso.
```
...
translated (by machine 🤖) as:
```
Introducir valores de configuración
```
...
translated (by machine 🤖) as:
```
La aplicación deberá tener el permiso de ubicación "siempre" para permanecer activa en segundo plano mientras se graba una pista.
```
...
translated (by machine 🤖) as:
```
Metro cuadrado (m²) / Hectárea (ha) / Kilómetro cuadrado (km²)
```
...
translated (by machine 🤖) as:
```
Pie cuadrado (ft²) / Acre (ac) / Milla cuadrada (mi²)
```
...
translated (by machine 🤖) as:
```
Sistema métrico: Metros / Kilómetros
```
...
translated (by machine 🤖) as:
```
La conexión a su cuenta Google se realiza en dos pasos: autenticación y autorización.

Es posible que vea dos ventanas emergentes seguidas durante el proceso de conexión; en ese caso, asegúrese de seleccionar la misma cuenta Google en ambas ocasiones.

%1$s Paso 1/2 - Autenticación

%2$s Paso 2/2 - Autorización
```
...
translated (by machine 🤖) as:
```
Se ha producido un error de autenticación inesperado (Google). Parece que la caché local de tu dispositivo está dañada.

Esto puede ocurrir si has revocado, restablecido o eliminado recientemente la conexión entre esta aplicación y tu cuenta (Google).

La caché del dispositivo debería actualizarse automáticamente cada hora.

Espera una o dos horas e inténtalo de nuevo.
```
