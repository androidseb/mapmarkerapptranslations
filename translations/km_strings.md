...
translated (by machine 🤖) as:
```
កម្មវិធីនេះប្រមូលទិន្នន័យទីតាំង ដើម្បីបើកមុខងារ 'ការថតបទ' ទោះបីជាកម្មវិធីបិទ ឬមិនប្រើក៏ដោយ។

ការប្រមូលទិន្នន័យទីតាំងនេះត្រូវបានប្រើទាំងស្រុងដើម្បីកត់ត្រាបទចូលទៅក្នុងសញ្ញាសម្គាល់របស់អ្នក ហើយកើតឡើងតែនៅពេលដែលអ្នកបានផ្តួចផ្តើមការថតបទយ៉ាងច្បាស់ប៉ុណ្ណោះ។

អ្នកអាចស្វែងរកព័ត៌មានបន្ថែមអំពីឯកជនភាពនៅក្នុងគោលការណ៍ឯកជនភាពដែលមាននៅតំណខាងក្រោម៖
%s
```
...
translated (by machine 🤖) as:
```
ការងើបឡើងវិញ៖ ការដំឡើងសុវត្ថិភាព
```
...
translated (by machine 🤖) as:
```
បង្កើតផែនទីទទេថ្មី និងអនុវត្តការកំណត់សុវត្ថិភាពដើម្បីការពារការគាំងបន្ថែមទៀត
```
...
translated (by machine 🤖) as:
```
ការដំឡើងសុវត្ថិភាពត្រូវបានអនុវត្ត ហើយនឹងមានប្រសិទ្ធភាពនៅពេលបើកដំណើរការលើកក្រោយ ឥឡូវនេះអ្នកអាចចេញពីកម្មវិធី ហើយបិទវាបាន។
```
...
translated (by machine 🤖) as:
```
នាំចេញទិន្នន័យផែនទីដែលមានក្នុងមូលដ្ឋាន
```
...
translated (by machine 🤖) as:
```
ការព្រមាន៖ ការនាំចេញការងើបឡើងវិញនឹងមានផ្នែកមួយ យ៉ាងហោចណាស់ឯកសារប្រព័ន្ធផ្សព្វផ្សាយពពកមួយត្រូវបានបាត់ នឹងមិនទាញយកទេ ហើយនឹងត្រូវបានជំនួសដោយឯកសារទទេកំឡុងពេលនាំចេញ។
```
...
translated (by machine 🤖) as:
```
វាហាក់ដូចជាកម្មវិធីរុករករបស់អ្នកមិនឆបគ្នាពេញលេញទេ - មានតែ Google Chrome ប៉ុណ្ណោះដែលត្រូវបានសាកល្បងថាត្រូវគ្នាយ៉ាងពេញលេញ
...
translated (by machine 🤖) as:
```
វាអនុញ្ញាតឱ្យអ្នកធ្វើសមកាលកម្មទៅម៉ាស៊ីនមេ WebDAV ដោយកំណត់រចនាសម្ព័ន្ធការចូលប្រើព័ត៌មានសម្ងាត់។
```
...
translated (by machine 🤖) as:
```
កម្មវិធីនេះនឹងត្រូវមានការអនុញ្ញាតទីតាំង 'ជានិច្ច' ដើម្បីបន្តសកម្មភាពក្នុងផ្ទៃខាងក្រោយខណៈពេលកំពុងថតបទ។
```
