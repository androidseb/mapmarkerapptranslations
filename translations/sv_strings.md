...
translated (by machine 🤖) as:
```
Detta begränsar appen till att bara komma åt filer som skapats av appen själv på din Google Drive. Välj det här alternativet om du bara planerar att synkronisera från ditt Google-konto utan medarbetare. Om du är osäker, välj det här alternativet, du kan enkelt ändra dig senare.
```
...
translated (by machine 🤖) as:
```
Synkroniseringen avbröts eftersom den överskred den tillåtna datavolymgränsen. För att öka eller ändra gränsen, gå till inställningarna.
```
...
translated (by machine 🤖) as:
```
Detta begränsar appen till att bara komma åt en app-dedikerad Dropbox-mapp. Om du är osäker, välj det här alternativet, du kan enkelt ändra dig senare.
```
...
translated (by machine 🤖) as:
```
Detta ger appen full åtkomst till din Dropbox. Välj det här alternativet om du vill att appen ska hitta filer i hela din Dropbox.
```
...
translated (by machine 🤖) as:
```
Skriv mer för att förfina din sökning
```
...
translated (by machine 🤖) as:
```
Synkroniseringen avbröts eftersom följande fil kunde hittas: %s. För att lösa detta kan du antingen återställa den saknade filen manuellt eller ändra appens inställningar för att inte ladda ner media under synkronisering.
```
...
translated (by machine 🤖) as:
```
Synkronisera medianedladdningspolicy
```
...
translated (by machine 🤖) as:
```
Ladda ner alla media under synkronisering
```
...
translated (by machine 🤖) as:
```
Ladda ner media efter synkronisering vid behov
```
...
translated (by machine 🤖) as:
```
Börja skriva historik för appkörningshändelser till loggfilen
```
...
translated (by machine 🤖) as:
```
Sluta skriva historik för appkörningshändelser till loggfilen och ta bort loggfilen
```
...
translated (by machine 🤖) as:
```
Exportera loggfilen så att den kan delas i felsökningssyfte
```
...
translated (by machine 🤖) as:
```
Inställningar för rendering av prestanda
```
...
translated (by machine 🤖) as:
```
Maximalt antal ritade flytande markörtitlar
```
...
translated (by machine 🤖) as:
```
Få tillgång till gamla kartdata
```
...
translated (by machine 🤖) as:
```
Se alternativ för att komma åt kartdata från de äldre versionerna av Map Marker.
```
...
translated (by machine 🤖) as:
```
Kartdatamigreringen lyckades: du bör nu kunna använda din gamla kartdata på den här nya versionen av Map Marker.
```
...
translated (by machine 🤖) as:
```
Kartdatamigrering misslyckades: åtkomst till din gamla kartdata är inte möjlig på den här nya versionen av Map Marker.
```
...
translated (by machine 🤖) as:
```
Detta öppnar en hjälpcenterlänk som förklarar hur du kan installera den gamla versionen av Map Marker utan att förlora din appdata.
```
...
translated (by machine 🤖) as:
```
Senaste säkerhetskopieringen gjordes för %s dagar sedan
```
...
translated (by machine 🤖) as:
```
Öppna och migrera en V2 molnkarta
```
...
translated (by machine 🤖) as:
```
Migrera en äldre molnkarta från Map Marker V2 (appversion från före 2023) och öppna den som en ny molnkarta
```
...
translated (by machine 🤖) as:
```
Legacy V2 molnkartas migrering lyckades!
```
...
translated (by machine 🤖) as:
```
Exportera den markerade markören som en fil
```
...
translated (by machine 🤖) as:
```
Vill du försöka importera en markör från följande textdata?
```
...
translated (by machine 🤖) as:
```
Välj ett alternativ för att spara den exporterade filen.
```
...
translated (by machine 🤖) as:
```
Den exporterade filen är stor och måste sparas med 'Spara som'.
```
...
translated (by machine 🤖) as:
```
Vill du avsluta och spara det här spåret?
```
...
translated (by machine 🤖) as:
```
Om du vill ge denna behörighet, gå till denna applikations behörighetsinställningar under dina systeminställningar.
```
...
translated (by machine 🤖) as:
```
Om du lämnar denna behörighet inaktiverad fungerar spårinspelning bara när appen är synlig i förgrunden.
```
...
translated (by machine 🤖) as:
```
Appen måste visa ett meddelande för att förbli aktiv i bakgrunden medan du spelar in ett spår.
```
...
translated (by machine 🤖) as:
```
Spårinspelningen (%s) avbröts innan den kunde sparas. Vill du spara den nu?
```
...
translated (by machine 🤖) as:
```
Vill du radera osparade spårinspelningar för alltid?
```
...
translated (by machine 🤖) as:
```
Det här spåret hade inga poäng registrerade och har helt enkelt kasserats.
```
...
translated (by machine 🤖) as:
```
Byt punkter med föregående/nästa-knapparna, flytta runt kartan för att placera punkterna och klicka på '-' för att ta bort en punkt. Klicka slutligen på spara för att spara det redigerade spåret.
```
...
translated (by machine 🤖) as:
```
%1$s timmar %2$s minuter %3$s sekunder
```
...
translated (by machine 🤖) as:
```
Aktivera premien från en kod som genereras från en annan enhet
```
...
translated (by machine 🤖) as:
```
Det gick inte att aktivera premien:
%s
```
...
translated (by machine 🤖) as:
```
Du har aktiverat gästpremiumåtkomsten på den här enheten
```
...
translated (by machine 🤖) as:
```
Skapa en premiumkod för att aktivera premium på en av dina andra enheter
```
...
translated (by machine 🤖) as:
```
Premiumkoden har skapats: %s
Använd den här koden från din andra enhet för att aktivera premien där.
```
...
translated (by machine 🤖) as:
```
Återställning: säker installation
```
...
translated (by machine 🤖) as:
```
Skapar en ny tom karta och tillämpar säkra inställningar för att förhindra ytterligare krascher
```
...
translated (by machine 🤖) as:
```
Den säkra installationen tillämpades och kommer att träda i kraft vid nästa lansering, du kan nu avsluta appen och avsluta den.
```
...
translated (by machine 🤖) as:
```
Exportera lokalt tillgänglig kartdata
```
...
translated (by machine 🤖) as:
```
Varning: återställningsexporten kommer att vara partiell, minst en molnmediafil saknas, kommer inte att laddas ner och kommer att ersättas med en tom fil under exporten.
```
...
translated (by machine 🤖) as:
```
Det verkar som om din webbläsare inte är helt kompatibel - bara Google Chrome testas som helt kompatibel
...
translated (by machine 🤖) as:
```
Detta gör att du kan synkronisera till en WebDAV-server genom att konfigurera åtkomstuppgifter.
```
...
translated (by machine 🤖) as:
```
Appen måste ha platsbehörighet "alltid" för att kunna vara aktiv i bakgrunden medan den spelar in ett spår.
```
...
translated (by machine 🤖) as:
```
Kvadratfot (ft²) / Tunnland (ac) / Kvadratmil (mi²)
```
...
translated (by machine 🤖) as:
```
Att ansluta till ditt Google-konto sker i två steg: autentisering och auktorisering.

Du kan se två popup-fönster i rad under anslutningsflödet – i så fall, se till att du väljer samma Google-konto båda gångerna.

%1$s Steg 1/2 - autentisering

%2$s Steg 2/2 - auktorisering
```
...
translated (by machine 🤖) as:
```
Spårregistreringen avvaktar med tillräckligt exakt GPS-positionering. Du kan justera noggrannhetsgränsen i appinställningarna.
```
...
translated (by machine 🤖) as:
```
Det gick inte att öppna popup-fönstret, troligtvis blockerat av din webbläsare. Kontrollera dina webbläsarinställningar för att tillåta popup-fönster på den här webbplatsen. De flesta webbläsare har en "tillåt"-knapp som visas runt adressfältet när popup-fönster blockeras.
```
